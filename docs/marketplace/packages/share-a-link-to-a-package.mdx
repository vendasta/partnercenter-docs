...
Before creating a **Buy it Now link**, make sure that the user you want to send the link to already exists. When viewing an account under **Partner Center > Accounts > Manage Accounts**, you can easily [add an existing user to an account or create one](/marketplace/users/add-existing-user-to-account) under the **Users** section. 
...
When viewing or editing a package from **Partner Center > Marketplace > Packages**, all published packages with the **Add to Shopping Cart** option on the **Purchase behavior** setting will have a **Direct link to the package**. This shortened URL can be copied by clicking the **Copy link** button. Following this URL will automatically [add the package to a customer's Shopping Cart](/marketplace/marketplace-packages/add-packages-to-cart) and direct them to the **Cart Contents** page, where they can [review the package and begin checkout](/marketplace/marketplace-packages/checkout-and-purchase-packages).
...
1. Go to **Partner Center > Marketing** and [create an email campaign](/marketing/create-email-campaign) or edit an existing campaign.
2. While editing an email in your campaign, click [**Insert Dynamic Component**](/marketing/email-dynamic-components) and select **Direct Store Link > Buy it Now Link.**
