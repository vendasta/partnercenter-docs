...
| "Autonomy Level" — four levels total (1 Suggestive, 2 Assistive, 3 Autonomous, 4 Agentic); default is Level 3: Autonomous, one below the top | **Included** — Shivapriya confirmed this directly in the live product, screenshots attached; see the Verification log below for the signed, dated attestation and the full ladder text. It doesn't appear in the `businessapp-docs` pages I checked, so that's a documentation gap worth flagging separately (tracked as `implementation-plan.md` WI-7 item 5), but the feature is real. Folded into Step 5 (where an employee's profile is first set up), with a short callback in Step 4. **Correction (2026-07-23):** earlier drafts of this outline and the lesson wrongly called Autonomous "the highest level" — it is one level below Agentic. Fixed everywhere. | Tag as `[VERIFIED IN-PRODUCT — Shivapriya, 2026-07-23, screenshot attached]` rather than `[DOC]` until the docs team adds a page for it. |
...
3. Lab: create the employee (name, avatar, profile), set its autonomy level, and write its role prompt — personality, voice, and behavioral rules the rest of the build runs on. Autonomous is the usual starting default, one level below the highest (Agentic): it manages full conversations, collects leads, and books appointments on its own, and every conversation stays reviewable. [VERIFIED IN-PRODUCT — Shivapriya, 2026-07-23, screenshot attached — not yet in businessapp-docs, flagged as a docs gap (implementation-plan.md WI-7 item 5); DOC custom-ai-employees/index.md setup steps 1-2 for the rest of the profile]
...
5. Lab: add one custom capability with a simple tool — an ID, a detailed description, a method and URL, the parameters it needs before it can act. **Runs against a real live endpoint (2026-07-23 fix, Cal's PR #796 item 4):** `CheckOrderStatus` calls `GET https://dummyjson.com/carts` with `order_number` (1-50) as a path parameter, a free public test API standing in for a real order system, verified live before writing it into the lesson. Path-vs-query parameter mechanics and the "no inline `{'{'}param{'}'}` templating in the URL" behavior confirmed against `creating-custom-capabilities.md`'s own field reference, not assumed. [DOC creating-custom-capabilities.md steps 2-4, "Example custom capability: Product information lookup", field reference for Method & URL / Parameters]
...
> **Shivapriya, 2026-07-23:** I personally checked this in the live product. When you create a new AI Employee, autonomy defaults to Level 3, Autonomous. That default is fixed — you can lower the level afterward if you want closer review while the employee is new, but the starting default itself isn't something you can configure. Two screenshots attached: [`learn-revamp/drafts/img/autonomy-level-3-in-product.png`](../drafts/img/autonomy-level-3-in-product.png), a live employee's Configure panel showing `Autonomy: Level 3: Autonomous`, and [`learn-revamp/drafts/img/autonomy-four-level-ladder.png`](../drafts/img/autonomy-four-level-ladder.png), the product's own "Levels of autonomy" info panel, which spells out all four levels verbatim:
>
> 1. **Suggestive** — lightweight guidance, no action taken. Provides suggestions or content recommendations but never acts on its own; supports human decision-making, doesn't replace it.
> 2. **Assistive** — on-demand support for single tasks. Helps complete straightforward actions with a click, always initiated by a human.
> 3. **Autonomous** — handles routine workflows. Acts automatically when triggered and can manage more advanced tasks with no human action needed. **This is the default for a new employee — and it is one level below the top, not the highest.**
> 4. **Agentic** — fully autonomous agent. Plans and executes complex, multi-step workflows independently; runs continuously or over time to achieve long-term outcomes without human involvement.
>
> Correcting my own earlier framing: I'd said Autonomous was "the highest level" in an earlier pass of this outline and in the lesson copy — that was wrong, Agentic sits above it. Fixed in both places.

This satisfies the `[VERIFIED IN-PRODUCT — Shivapriya, 2026-07-23, screenshot attached]` tag used in the source-map table and beat 3 above, and the full ladder text is itself good raw material to hand to the businessapp-docs team as a documentation source once WI-7 item 5 gets flagged to them. Standing flag per Cal's rule: this claim should graduate to a real `[DOC]` citation once businessapp-docs covers the autonomy default — tracked as `implementation-plan.md` WI-7 item 5.

## Changelog

- **v7** (July 23, 2026): Cal's PR #796 item 4 resolved — beat 5's `CheckOrderStatus` tool now runs against a real, live-verified endpoint (`GET https://dummyjson.com/carts`, `order_number` as a path parameter, free/public/no-auth) instead of staying a hypothetical. Capability description expanded per Shivapriya's request ("explain it in detail"). Path-vs-query parameter mechanics checked against `creating-custom-capabilities.md`'s field reference first — no inline `{'{'}param{'}'}` URL templating exists in the real UI, parameters are configured separately with a location (path/query/body), so the lesson describes it that way rather than guessing a syntax. Marks `implementation-plan.md` WI-7 item 7 done.
- **v6** (July 23, 2026): Correction — the v5 wording wrongly called Autonomous "the highest level." Shivapriya's screenshots confirm four levels total (Suggestive, Assistive, Autonomous, Agentic); Autonomous is Level 3, one below Agentic. Fixed in the lesson and everywhere in this outline. Full ladder text logged in the Verification log as `[VERIFIED IN-PRODUCT]` source material.
- **v5** (July 23, 2026): Autonomy-level wording softened to survive a UI change, per Cal's PR #796 review. `[CONFIRMED IN-PRODUCT]` tags upgraded to the new `[VERIFIED IN-PRODUCT]` format with Shivapriya's signed, dated, screenshot-backed attestation (see Verification log). Feeds WI-7 items 5-7 in implementation-plan.md.
