...
5. businessapp-docs: the AI Employee autonomy level (four levels: Suggestive, Assistive, Autonomous, Agentic; Level 3 Autonomous is the default) isn't documented anywhere in businessapp-docs — currently rests on an in-product check only. **Done (2026-07-23, branch `docs/pr796-autonomy-followups`):** lesson wording in custom-employee-lab.mdx corrected and softened to survive a UI change ("Autonomous is the usual starting default, one level below the highest (Agentic)" — an earlier pass of this fix wrongly said "highest level," since corrected everywhere); Shivapriya's signed, dated, screenshot-backed `[VERIFIED IN-PRODUCT]` attestation logged in `drafts/OUTLINE-ai-workforce-train-and-custom-lab.md` (Verification log section), including the full four-level ladder text verbatim from the product, per CalCooper's PR #782 tagging proposal. **Still open:** flag the actual docs gap to the businessapp-docs team so this graduates to a real `[DOC]` citation.
...
7. custom-employee-lab.mdx: the Required line promised "use the example in this step," but `CheckOrderStatus` was illustration-only, not a runnable API. **Done (2026-07-23, branch `docs/pr796-autonomy-followups`):** wired the lab to a real, verified live endpoint — `GET https://dummyjson.com/carts` with `order_number` as a path parameter (free, public, no auth) — with a detailed capability description and an explanatory paragraph noting it's a stand-in, not a real Vendasta/client order system. Path-vs-query parameter mechanics confirmed against `creating-custom-capabilities.md`'s field reference before writing, per CalCooper's PR #796 review.
