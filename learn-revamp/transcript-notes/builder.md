...
7. **Custom tool/webhook building for an AI Employee capability:** add a tool inside a capability, name it, give the AI a description of what it's for, and configure it as a webhook connector. Confirmed limitations: only **GET and POST** request types are supported today; **no native OAuth 2 support** — only static API token/API key authentication works. Setup mechanics: paste the receiving platform's webhook endpoint URL, then define each parameter with its type (body vs. other), variable type, whether it's required, and whether its value is **set by the AI dynamically** (with a short guiding sub-prompt) or fixed/static. The tool must also be explicitly named/referenced inside the employee's role prompt so the AI knows when and how to use it.
