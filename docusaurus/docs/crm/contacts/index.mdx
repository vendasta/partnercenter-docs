---
title: Contacts
description: "Manage contacts in the Partner Center CRM: import and export in bulk, schedule campaigns, bulk delete, and create custom contact fields."
sidebar_position: 1
brand: vendasta
product: crm
audience: partners
...
kb_id: KB-00003
answer_snippet: "Contacts in the Vendasta CRM, at CRM → Contacts in Partner Center, store the people you sell to and work with. You can import and export contacts in bulk, schedule email campaigns, and add custom fields."
doc_owner: crm
...
Contacts in the Vendasta CRM, at `CRM` → `Contacts` in Partner Center, store the people you sell to and work with. You can import and export contacts in bulk, schedule email campaigns, and add custom fields.
...
4. **Send your first marketing email**
   ...
   - Navigate to `Partner Center` → `Contacts` and select contacts
...
In the Vendasta CRM, you can efficiently add or update multiple contacts and companies at once by importing them in a single file. This streamlines contact management and ensures data accuracy through proper field mapping during the import process.
...
**Step 1:** Navigate to `Partner Center` → `Contacts` and select the contacts you wish to send the campaigns to. Click `Actions` → `Add to Campaign`.
...
<details>
<summary>How do I export contacts from the CRM?</summary>

Go to `CRM` → `Contacts`, click the top-left checkbox, then click the blue `select all [N] contacts` link. Click `Actions` → `Export`.
</details>

<details>
<summary>How do I create a custom contact field?</summary>

Go to `Administration` → `CRM objects`, select `Contact`, and click `Create field`. Choose the field type and set its label and external identifier. The external identifier can't be changed after you save.
</details>

<details>
<summary>Why did my import update existing contacts instead of creating new ones?</summary>

The CRM matches imported rows to existing contacts by contact ID, external ID, email, or phone number. When it finds a match, it updates that contact instead of creating a duplicate. You can choose to skip existing records during the import.
</details>

<details>
<summary>How do I delete contacts in bulk?</summary>

Select up to 100 contacts at a time in `CRM` → `Contacts`, then click `Actions` → `Delete` and type `Delete` to confirm. Deletion is permanent, so export any records you need first.
</details>