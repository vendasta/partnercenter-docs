...
![CSV Upload Screen](./img/crm/contacts/start-an-import.png)
...
1. **Export existing contacts**
   ...
   - Select contacts you want to update (or use "Select All")
   - Click `Export` to download current contact data
...
## Contact profile action icons (CRM → Contacts)

When viewing a contact in **Partner Center** (`CRM` → `Contacts`) or **Business App** (`CRM` → `Contacts`), a set of action icons appears at the top of the **Contact profile**. These icons allow you to quickly communicate with the contact or manage their details without leaving the profile.

### Send a message

Use this action to send a message directly to the contact using one of the supported channels.

You can send:
- SMS (A2P registration required)
- Facebook message (available only for contacts created via Facebook Messenger)
- WhatsApp message
- Email

Messages sent using this option are logged in Conversations

**Note:** SMS template customization is supported. The contact must have a valid phone number.

### Make a phone call

Use this action to initiate a phone call with the contact.

### Book a meeting

The Book a Meeting action allows you to share your meeting availability with the contact.

- Displays your available scheduling links (for example, 15-minute, 30-minute, or custom meeting types)
- Sends a booking link so the contact can select a time that works for them
- Meetings booked through this flow are tracked in the CRM

**Important:**  
Meeting invitations sent from the CRM can include customized email content. This customization does not apply to meetings booked using your public scheduling link.

### Send email

Use this action to send a one-to-one email directly to the contact.

This option is ideal for quick follow-ups without creating a campaign.

### More options

The More options (three-dot) menu provides additional actions.

Currently, this menu allows you to:
- **Edit contact** – Update the contact’s information, including name, email address, phone number, tags, and custom fields

Additional options may appear as new CRM features are introduced.
