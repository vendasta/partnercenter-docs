...
2. Click the checkbox in the top-left corner to select the current page's records. A banner confirms how many are selected, for example "25 of 143 selected"
3. Click the blue `Select all [N] contacts` link in the banner to select every matching record instead of just the page. The banner then confirms all matching records are selected and offers a link to clear the selection
