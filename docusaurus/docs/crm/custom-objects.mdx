...
Each custom object type has its own [field layout](/administration/data-management/crm-objects/field-layouts), so you can group and order its fields independently of contacts, companies, and opportunities.
...
<details>
<summary>Is there a limit to how many custom objects an account can have?</summary>

Yes. Each account can have up to 3 custom objects. This limit applies whether the object is created manually or rolled out through an [account template](/accounts/manage-accounts/account-templates) — if applying a template would push an account past the limit, the application fails for that account with a clear error instead of failing silently.
