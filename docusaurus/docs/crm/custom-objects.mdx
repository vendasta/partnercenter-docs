...
Yes. Each account can have up to 3 custom objects. This limit applies whether the object is created manually or rolled out through a [blueprint](/accounts/manage-accounts/blueprints) — if applying a blueprint would push an account past the limit, the application fails for that account with a clear error instead of failing silently.
