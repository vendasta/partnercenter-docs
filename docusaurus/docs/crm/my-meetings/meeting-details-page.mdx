...
- Generate and copy an external meeting link
...
3. Choose the link tab you want to use:
   - **Internal link**: Copy the internal meeting URL. A hint is shown below the field.
   - **External link**: Turn on the toggle to generate an external URL, then copy the URL. A hint is shown below the field.
4. Optional: set a custom start time to direct viewers to a specific point in the meeting.

### External link behavior

In the **External link** tab:
- When the toggle is off, no external URL is shown.
- When the toggle is on, an external URL is generated and available to copy.
- When enabled, the tab shows a green status icon so you can quickly see that external sharing is on.
- Turning the toggle off shows a success snack confirming the external link is off.
- Turning the toggle back on restores the same external URL for that meeting.
- Copying the external URL shows the same copy confirmation snack as the internal link.
