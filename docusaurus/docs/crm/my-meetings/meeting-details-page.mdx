...
- Turn external sharing on or off and copy an external share link
...
3. Select a link tab:
   - **Internal link**: Copy the internal meeting link. You can also set a custom start time to direct viewers to a specific point in the meeting.
   - **External link**: Turn on the external link toggle to generate an external URL, then copy it.
4. If the external link is enabled, the **External link** tab shows a green indicator so you can quickly see that public sharing is active.
5. To stop external sharing, turn off the external link toggle. The change is applied immediately, and turning it back on restores the same external URL.
