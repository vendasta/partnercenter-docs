...
- Copy an internal share link to the meeting
- Generate and copy an external share link when needed

Visibility settings and link tabs work together. Visibility controls who can access the meeting inside your company and whether activity is logged in the CRM. The link tabs control whether you share an internal link or an external link.
...
3. Use the link tabs based on your sharing goal:
   - **Internal link** tab: Copy the internal URL for people in your company.
   - **External link** tab:
     - When external sharing is off, no external URL is shown.
     - Turn on external sharing to generate an external URL.
     - When enabled, a green icon appears on the External link tab so you can quickly confirm it is on.
     - Copy the generated external URL as needed.
4. Optional behavior to know:
   - Turning external sharing off shows a success notification.
   - Turning external sharing back on restores the same external URL for that meeting.
   - Copy notifications for internal and external links use the same behavior.
5. Optionally, set a custom start time to direct viewers to a specific point in the meeting. This is useful if you want to share a specific section of the meeting.
