...
- Use **Internal link** and **External link** tabs
- Copy an internal or external shareable link to the meeting
...
3. Select a link tab:
   - **Internal link**: Use this tab for internal sharing only.
   - **External link**: Use this tab to share a public-facing URL.
4. If you are using the **External link** tab, turn the toggle on to generate the external URL. If the toggle is off, no external URL is available.
...
- The visibility controls are unchanged and still apply separately from link tab selection.
- When the **External link** toggle is on, the tab shows an enabled indicator so you can see that external sharing is active.
- If you turn the external toggle off, the external URL is disabled and a success message confirms that external sharing is off.
- If you turn the external toggle back on later, the same external URL is reused.
- Copy confirmation for external links uses the same snack behavior as internal link copy.
