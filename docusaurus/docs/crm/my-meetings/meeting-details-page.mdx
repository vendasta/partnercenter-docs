...
- Copy an internal link for your organization
- Turn an external link on or off, then copy it when enabled
...
Use the **Share** modal to control visibility and copy meeting links.

### Visibility settings

The visibility section works the same way as before. You can choose:
...
The **Internal link** tab keeps the existing sharing flow:

1. Select **Share** on the Meeting Details page.
2. Open **Internal link**.
3. Copy the generated internal URL.
4. Optionally, set a custom start time to direct viewers to a specific point in the meeting.

### External link tab

The **External link** tab lets you manage external sharing for a meeting:

1. Select **Share** on the Meeting Details page.
2. Open **External link**.
3. Turn the external link toggle on to generate an external URL.
4. Copy the generated external URL.

When external sharing is enabled:

- A green status icon appears on the **External link** tab so you can quickly see it is on.
- Copy confirmation behavior matches the internal link copy flow.

When external sharing is disabled:

- No external URL is shown.
- A success message confirms that the external link is turned off.

If you turn external sharing back on later, the same external URL is reused.
