...
- Share the meeting using an **Internal link** or **External link**
...
2. Choose a visibility option:
   ...
   The visibility section of the Share modal works the same as before.
3. Use the link tabs to choose how you want to share:
   - **Internal link**: Copy the internal meeting URL. This tab includes help text below the URL field.
   - **External link**: Turn on the toggle to generate an external URL, then copy it to share externally. This tab also includes help text below the URL field.
...
- When the **External link** toggle is off, no external URL is available.
- When the **External link** toggle is on, an external URL is generated and can be copied.
- When an external link is enabled, the **External link** tab shows a green status icon.
- Turning off the external link applies immediately and does not require a confirmation modal.
- If you turn off the external link, a success snackbar confirms it was turned off.
- If you turn the external link back on later, the same URL is reused.
- Copying the external link uses the same copy confirmation snackbar as the internal link.
