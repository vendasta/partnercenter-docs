...
- Share the meeting with an **Internal link** or **External link**
...
3. In the link section, choose the tab you want:
   - **Internal link**: Use the existing internal share URL. A hint appears below the URL field.
   - **External link**: Turn on the external link toggle to generate a URL. A hint appears below the URL field, and when this is on, a green indicator appears on the **External link** tab.
4. Copy the link you need. Copying either link shows a confirmation message.
5. If you turn off the external link toggle, there is no external URL and a success message confirms the link is off.
6. If you turn the external link back on later, the same external URL is reused.
7. Optionally, set a custom start time to direct viewers to a specific point in the meeting. This is useful if you want to share a specific section of the meeting.
