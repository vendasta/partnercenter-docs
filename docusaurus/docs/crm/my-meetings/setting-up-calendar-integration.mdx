...
Once your calendar is connected, busy time from it appears on your calendar view as **Blocked** time, so you don't accidentally book over it. Turn **Show blocked time slots** on or off from `My Meetings` → `Meeting Settings` → `Calendar settings` — it's on by default, and it's a personal setting: each person's toggle controls only what they see. This is visual only — connected-calendar busy time keeps being excluded from your bookable availability whether the toggle is on or off.
