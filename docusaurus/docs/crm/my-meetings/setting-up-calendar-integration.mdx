...
## Showing or hiding blocked time

Busy time from your connected calendar appears on the `My Meetings` calendar as read-only `Blocked` time, so you can see when you're unavailable.

To hide it, turn off `Show blocked time slots` at the bottom of the calendar sidebar. It's on by default, and your choice is saved for the next time you sign in.

:::note
This setting is yours alone and changes only what you see, including the blocked time of team members on shared views. It does not affect booking: busy time from your connected calendar is always excluded from the slots invitees can book, whether the toggle is on or off.
:::
