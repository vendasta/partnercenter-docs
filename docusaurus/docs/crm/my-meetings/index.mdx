...
If the event type you select has a daily meeting limit configured and the date you choose has already reached it, a warning icon appears in the dialog next to the date and time. Hover the icon for a message explaining that the booking exceeds the host's daily limit for that day. This is informational only — you can still complete the booking after seeing the warning. The icon only appears when a daily limit is set on the event type; there's no change to the dialog when no limit is configured.
