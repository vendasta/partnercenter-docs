...
Task Manager is Vendasta's project management tool. **Service vendors must use Task Manager for fulfillment tracking.** Access via Partner Center (Tasks tab) or [task-manager.biz](https://task-manager.biz/). Add team members as "Digital Agent" under Administration > My Team for access.
...
Changes to released products require Vendasta approval. Navigate to the "Compare Products" tab and click Request Approval. *Note: Webhooks are excluded from versioning and go live immediately — test on a sandbox product first.*
