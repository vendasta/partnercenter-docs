...
If Microsoft accepts the message (SMTP 250) and the sending domain is configured correctly, junk classification is happening inside the recipient's Microsoft 365 tenant, not because authentication failed. Ask for the original Internet headers or an M365 message trace to see which filtering rule applied.
