...
description: "How sender domains and SPF, DKIM, and DMARC records affect email deliverability, plus best practices, error codes, and spam troubleshooting."
brand: vendasta
product: marketing
audience: partners
content_type: overview
kb_id: KB-00058
answer_snippet: "Email deliverability for Partner Center emails and Campaigns depends on your sender domain, authentication records, content, and list health. The platform sends from a verified default domain, or from your own domain once you add SPF, DKIM, and DMARC records and update Marketing → Email settings."
doc_owner: marketing
...
Email deliverability for Partner Center emails and Campaigns depends on your sender domain, authentication records, content, and list health. The platform sends from a verified default domain, or from your own domain once you add SPF, DKIM, and DMARC records and update `Marketing` → `Email settings`.

This article covers how the platform handles sender domains, best practices, and troubleshooting to improve your email deliverability when using Campaigns.
...
Spam laws and regulations require precautions to prevent emails from being flagged as spam. To help deliver your emails successfully, Vendasta makes automatic adjustments to optimize your email delivery.
...
1. Update your DNS records with proper authentication: one SPF (TXT), **two** DKIM (CNAME), and one DMARC (TXT)
2. Go to `Partner Center` → `Marketing` → `Email settings`
3. In the `Advanced email settings` card, set `Sender domain` to `Use my own email domain` and enter your `Email address`, then select `Save` on that card
...
3. **Update sender settings** - Go to `Partner Center` → `Marketing` → `Email settings` and set `Sender domain` and `Email address` in the `Advanced email settings` card, then select `Save` on that card
...
<details>
<summary>Why can't I send emails from my Gmail address?</summary>

Free email addresses such as gmail.com, outlook.com, and yahoo.com can't be verified because you don't own those domains, and they won't save as a sender address. The platform sends from your address at a verified default domain instead, so your emails pass spam filters.

</details>

<details>
<summary>Where do replies go when my emails are sent from the default domain?</summary>

All replies are automatically forwarded to the email address you used to sign up for the platform.

</details>

<details>
<summary>What do I enter in Advanced email settings to use my own domain?</summary>

Go to `Partner Center` → `Marketing` → `Email settings`. In the `Advanced email settings` card, set `Sender domain` to `Use my own email domain`, enter an email address on your custom domain, and select `Save`. Add one SPF (TXT), two DKIM (CNAME), and one DMARC (TXT) record to your DNS, and allow up to 72 hours for the changes to propagate.
