---
title: "How do I send marketing emails?"
description: "Send email campaigns to CRM contacts, companies, and lists, through automation steps, or to account lists and individual users in Partner Center."
...
content_type: how-to
kb_id: KB-00026
answer_snippet: "To send marketing emails with Campaigns in Partner Center, add CRM contacts or companies to a campaign, start a campaign from a CRM list, or use automation steps. You can also send campaigns to account lists or individual account users."
doc_owner: campaigns
...
To send marketing emails with Campaigns in Partner Center, add CRM contacts or companies to a campaign, start a campaign from a CRM list, or use automation steps. You can also send campaigns to account lists or individual account users.
...
## What are the recommended ways to send campaigns? {#primary-methods-recommended}

### 1. Send to CRM contacts

The primary and recommended way to send campaigns is through CRM contacts, which provides full field support and tracking:
...
- Suited to account-based marketing and business-level messaging
...
Set up automated campaigns with business logic:
...
## Why should I send campaigns through the CRM? {#benefits-of-crm-based-sending}
...
## Should I target contacts or companies? {#choosing-between-contact-vs-company-targeting}
...
## What other ways can I send campaigns? {#alternative-methods}
...
## What are some examples of automated campaigns? {#smart-automation-examples}
...
## How do I configure campaign sending options? {#campaign-configuration-options}
...
## How do I monitor campaign performance? {#monitor-campaign-performance}
...
For detailed guidance on analyzing your campaign results, see [Track Email Campaign Performance](./track-email-campaign-performance.mdx) for metrics and optimization strategies.

:::tip Recommendation
Use CRM-based methods whenever possible for better data integration, tracking capabilities, and automation potential. The CRM approach provides more granular control over contact management.
:::
...
<details>
<summary>How do I send a campaign to a list of contacts?</summary>

Go to `Partner Center` → `CRM` → `Lists` and create or select a contact list. Click `Three dots` → `More action`, select `Start Campaign for Contact` or `Send Email to Contact`, then choose your campaign and start sending.
</details>

<details>
<summary>How do I send a campaign automatically, for example when someone submits a form?</summary>

Go to `Partner Center` → `Automations` and create a workflow with a trigger such as a form submission. Add a `Start Campaign for Contact` or `Send Email to Contact` step to send the campaign or email when the trigger fires.
</details>

<details>
<summary>What is the difference between the automation email steps?</summary>

`Start Campaign for Contact` launches a campaign targeting an individual contact, and `Start Campaign for Company` initiates a campaign for the entire company. `Send Email to Contact/Company` sends a direct email to the selected contact or company, and the `Campaign Step` uses an existing campaign as a building block within a workflow.
</details>

<details>
<summary>How do I send a campaign to an account list?</summary>

Go to `Partner Center` → `Marketing` → `Campaigns`, find your campaign, and click the **3-dot menu**. Select **Add account list to campaign**, then configure the sending options.
</details>

<details>
<summary>Who are campaign emails sent from?</summary>

By default, campaigns use the sender in your configured email settings, and you can override it with custom sender details for a campaign. When an account has an assigned salesperson, campaign emails go out from the salesperson's email address and name instead. To send from your partner-level sender, remove the salesperson assignment from the account before starting the campaign.
</details>

<details>
<summary>Where do replies to campaign emails go?</summary>

Campaign replies go to the sender address, which is either the assigned salesperson or your partner-level sender. The `Reply Address` field in `Marketing` → `Email settings` → `Sender and reply settings` applies only to system emails, such as Daily Digest notifications.
</details>

:::tip Pro Tip
All email activity from CRM-based campaigns is automatically tracked in the contact and company records, enabling follow-up automation and lead scoring based on engagement patterns.
:::