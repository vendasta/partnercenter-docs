...
:::note
The `Wait [n] days before starting` / `after previous event` field on each email is only available while the campaign is in `Draft`. Once a campaign is published, email timing is locked along with the event order, and the field no longer appears on the campaign details page.
:::

To change the timing of a published campaign:

1. Click `Actions` → `Copy` to create a draft copy of the campaign.
2. Adjust the `Wait` value on each email in the copy.
3. Archive the original campaign and publish the copy.
