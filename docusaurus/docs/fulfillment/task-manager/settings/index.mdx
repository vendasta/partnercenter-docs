...
1. Navigate to `Fulfillment` → `Task Manager`
...
You can configure the Task Manager `Send from` email address by navigating to `Fulfillment` → `Task Manager` → `Settings` → `Email Settings`.
...
You can customize your Task Manager experience:
...
1. Navigate to `Fulfillment` → `Task Manager` → `Settings` → `User settings`
2. Click `Edit`
