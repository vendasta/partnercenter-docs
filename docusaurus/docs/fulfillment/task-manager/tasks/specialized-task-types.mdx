...
- **Reputation AI**: Address concerns publicly and professionally
...
1. Navigate to `Task Manager` → `Tasks`
...
:::info
Approval emails are sent to Business App users and notification recipients who have **Review Response Approval** enabled in Business App > Settings > Notification Settings > Reputation AI.
:::
...
2. Go to `Settings` → `Task generation` → `Reputation AI`
...
1. Navigate to `Fulfillment` → `Task Manager` → `Accounts`
...
- Enable "Review Response Approval" under Reputation AI
...
You can create review response templates for different star ratings and business categories in Settings > Task generation > Reputation AI. For other types of custom tasks, you'll need to create them manually or use the auto-generation features for standard task types.
