...
You can schedule draft posts directly from draft post cards. The scheduling action is available in desktop and mobile toggle views.

Scheduling follows these validation rules:
- The post must be in `Drafts`.
- The selected date and time must be in the future.
- At least one valid social network connection is required.

When scheduling is unavailable, the interface shows one of these messages:
- `Please select a future date and time to schedule this post`
- `Please ensure the scheduled time is in the future and at least one social account is connected`
