...
<details>
<summary>Why does activating a package fail with an "editions not supported" error?</summary>

This usually means a SKU in the package was stamped with editions when the package was built, and those editions were later turned off on the SKU. The package still carries the old editions stamp, so activation fails. Recreate or update the package so it no longer references unsupported editions, then activate again. If it keeps failing, contact support with the SKU and package details.
