---
title: "How do I fix common Marketplace package problems?"
sidebar_label: "Troubleshooting & Advanced Guide"
sidebar_position: 3
description: "Fix Marketplace package problems in Partner Center: store visibility, duplicates, broken links, pricing changes, multi-market sync, and support escalation."
...
kb_id: KB-00083
answer_snippet: "Most Marketplace package problems come from package setup: a missing store category, the wrong market, duplicate display settings, the purchase behavior, or the pricing model. Find your symptom below and follow the fix."
...
Most Marketplace package problems come from package setup: a missing store category, the wrong market, duplicate display settings, the purchase behavior, or the pricing model. Find your symptom below and follow the fix.

## Why isn't a product visible or available to activate? {#product-visibility-and-activation-constraints}

### Why isn't my product appearing when `Available in store` is set to Yes? {#available-in-store-is-set-to-yes-but-the-product-isnt-appearing}
...
### Why is my custom product missing from Order Products? {#custom-product-missing-from-order-products}
...
### Why can't an account activate a product anymore? {#country-specific-product-activation}
...
### Why isn't the Review Widget Carousel option available? {#review-widget-carousel-requires-review-widget-pro}
...
### How do I find a product's SKU? {#finding-a-products-sku}
...
## How do I fix common package issues? {#common-package-visibility-issues}

Package visibility problems are the most common package issues. Use the symptoms, root causes, and solutions below to get your packages in front of customers.

### Why aren't my packages appearing in the Business App Store? {#issue-1-packages-not-appearing-in-business-app-store}
...
### Why do duplicate packages appear in the Public Store? {#issue-2-duplicate-packages-in-public-store}
...
### Why aren't my package links working? {#issue-3-package-links-not-working}
...
### Why did my package pricing change after editing? {#issue-4-package-pricing-changed-after-editing}
...
### Why is Package Profit showing as unavailable? {#package-profit-showing-as-unavailable}
...
### Why isn't the retail price updating after I edit a package? {#retail-price-not-updating-after-editing-a-package}

In the package editor, updating the retail price of an item inside a package does not automatically recalculate and display the updated total in the list and grid views immediately. This is a known behavior. To apply the updated price:
...
Note: bulk pricing updates across multiple packages are not supported. Each package must be updated individually.

### Why isn't my package appearing when I add it to an invoice? {#package-not-appearing-when-adding-it-to-an-invoice}
...
## How do I fix packages that differ across markets? {#advanced-troubleshooting-scenarios}
...
## When should I contact support about a package issue? {#support-resources-and-escalation}
...
Packages must use the line-item pricing model to be compatible with invoices and the Shopping Cart checkout. Open and save a legacy package in the package editor to migrate it automatically, and it should then appear when you create invoices.

</details>

<details>
<summary>Why is my package showing a different price than I set up?</summary>

Existing packages migrate to the line-item pricing model the first time they are edited and saved, which redistributes the total price across line items. The overall package price is preserved; review and adjust the item-level prices in `Marketplace` → `Packages` if needed.

</details>

<details>
<summary>Can I update pricing across multiple packages at once?</summary>

No. Bulk pricing updates across multiple packages are not supported, so each package must be updated individually.

</details>

<details>
<summary>How do I find a product's SKU?</summary>

Go to `Marketplace` → `Discover Products` and click into the product. The SKU is the string at the end of the URL in your browser, in the format `MP-` followed by alphanumeric characters.

</details>

<details>
<summary>Why can't I find a product to add to an order?</summary>

If a custom product is missing from `Order Products`, add an address to the account's business profile. Some products are also restricted to specific countries, so the account's business profile country must match the product's country availability.
