...
**Creating new users**
1. In `Partner Center` → `Accounts`, start a new user from `Manage Users` → `Create User`, or open `Manage Accounts` → `[Account Name]` → `Users` if you are already working inside that account.
2. Complete user form ensuring email domain matches Google Workspace domain
3. Add user to target account:
   - Select the edit icon → `Edit Permissions` → `Add Account`
   - Locate target account and select checkbox
   - Click `Add Accounts`
4. Repeat for all Google Workspace users
