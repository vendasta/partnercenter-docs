---
title: "How do I set up and manage Google Workspace for my clients?"
sidebar_label: "Google Workspace guide"
sidebar_position: 3
description: "Set up, transfer, and cancel Google Workspace for clients in Partner Center, and manage users, DNS, and SSO for Google Workspace."
...
content_type: how-to
kb_id: KB-00020
answer_snippet: "To set up Google Workspace for a client, open the account in Accounts → Manage Accounts in Partner Center and click Create Order → Add Google Workspace. To move an existing subscription to Vendasta, use Add Google Workspace Transfer with the client's transfer token."
...
To set up Google Workspace for a client, open the account in `Accounts` → `Manage Accounts` in Partner Center and click `Create Order` → `Add Google Workspace`. To move an existing subscription to Vendasta, use `Add Google Workspace Transfer` with the client's transfer token. This guide covers setup, transfers in and out of Vendasta, and SSO.

## What is the Google Workspace integration? {#overview-of-google-workspace-integration}
...
Google doesn't allow transfers to a reseller that hosted the same Google Workspace within the last 18 months. Verify the client's previous hosting history before initiating a transfer.
...
For visual guidance through the setup process, watch this walkthrough:
...
<details>
<summary>What is Vendasta's reseller ID for a Google Workspace transfer?</summary>

Give your client Vendasta's Reseller ID, `C02e5br85`. The client then generates a transfer token at [admin.google.com/TransferToken](https://admin.google.com/TransferToken).

</details>

<details>
<summary>How do I move a client's Google Workspace out of Vendasta?</summary>

Cancel the Google Workspace product. The client's emails and data stay intact, and their Google Workspace admin gets an email to set up billing directly with Google, which they must do within 7 days to avoid interruption.

</details>

<details>
<summary>How do I cancel one Google Workspace seat instead of all of them?</summary>

Cancel the seat and also contact support@vendasta.com, since removing individual seats may need backend adjustments.

</details>

<details>
<summary>Why can't I transfer a client's Google Workspace to Vendasta?</summary>

Google blocks transfers to a reseller that hosted the same Google Workspace in the last 18 months. Also confirm the transfer token is valid and that the client has admin access to the domain.
