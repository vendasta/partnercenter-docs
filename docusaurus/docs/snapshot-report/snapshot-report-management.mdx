...
description: "Refresh and save Snapshot Reports, understand their fees, add packages to report buttons, and use forms and campaigns to turn reports into leads."
...
kb_id: KB-00054
answer_snippet: "You get more value from Snapshot Reports in Partner Center by refreshing them only when needed, saving them as PDFs, adding packages to report buttons, and using forms that create reports automatically as lead magnets. Each new or refreshed report costs $2."
...
## How do I get the most value from Snapshot Reports? {#what-is-maximizing-snapshot-report-value}

You get more value from Snapshot Reports in Partner Center by refreshing them only when needed, saving them as PDFs, adding packages to report buttons, and using forms that create reports automatically as lead magnets. Each new or refreshed report costs $2.

## Why focus on value maximization?

- **Lead generation**: Use reports as lead magnets on your website
- **Sales follow-up**: Automated workflows follow up with prospects while you focus on closing
- **Cost control**: Creating and refreshing reports only when you need them avoids unnecessary fees

## Strategic report management

### When should I refresh a Snapshot Report? {#when-to-refresh-reports}
...
### How do I refresh a Snapshot Report? {#refresh-methods}
...
**Steps:**
1. Navigate to `CRM` → `Companies` and select the desired company
...
### How do I save a Snapshot Report as a PDF? {#saving-reports-as-pdf}
...
### How do I add packages to reports? {#adding-packages-to-reports}
...
### How do I capture leads with a Snapshot Report form? {#automated-lead-capture-with-forms}

Turn Snapshot Reports into lead magnets that work 24/7:

**Setup Process:**
1. **Create Form**: Go to `Marketing` → `Forms` → `Create form`. Select the `Acquisition Widget` template.
...
3. **Configure Automation**: 
   - Go to the form's `Automation` tab
...
- **Free offer**: Prospects get a digital presence assessment at no direct cost to them
...
Reports generate automatically when forms are submitted. The marketing email containing the report link sends 24 hours later, giving the report time to gather data.
</details>
...
Creating a report generates entirely new data analysis for a business. Refreshing updates existing report data with current information. Both processes involve the same analysis and data collection, which is why they cost the same amount.
</details>

<details>
<summary>How do I run a new Snapshot Report for an account that already has one?</summary>

Refresh the existing report. Go to `Partner Center` → `Accounts` → `Manage Accounts`, click the `Snapshot icon` next to the account, and click `Refresh` at the bottom of the scorecard, or use the 3 dots next to the report in `CRM` → `Companies`. The refresh option only appears if the report is older than 7 days.
</details>

<details>
<summary>How do I set up a form that prospects fill out to get a Snapshot Report?</summary>

Go to `Marketing` → `Forms` → `Create form` and select the `Acquisition Widget` template. On the form's `Automation` tab, configure a marketing email campaign and turn on the automation. When someone submits the form, a Snapshot Report generates automatically and the lead appears in your CRM.
</details>
