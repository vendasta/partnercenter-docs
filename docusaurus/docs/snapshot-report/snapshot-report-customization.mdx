---
title: "How do I customize the Snapshot Report?"
sidebar_label: Customization And Configuration
description: "Customize Snapshot Reports in Partner Center: reorder or remove sections, edit messages, add custom headers and banner images, and set the default contact."
...
content_type: how-to
kb_id: KB-00072
answer_snippet: "You can customize the Snapshot Report in Partner Center by reordering or removing sections, editing messages, adding custom HTML headers and banner images, and setting the Need Assistance? contact. Template changes apply only to new and refreshed reports."
...
# How do I customize the Snapshot Report? {#snapshot-report-customization-and-configuration}

You can customize the Snapshot Report in Partner Center by reordering or removing sections, editing messages, adding custom HTML headers and banner images, and setting the Need Assistance? contact. Template changes apply only to new and refreshed reports.
...
**For a Single Report:**
1. Open the account's Snapshot Report in **Edit Report** mode
2. Beside each section, use the checkbox to control whether that section is visible to the client
3. Uncheck any section that does not apply to the business (for example, Ecommerce for a business that does not sell online) before sharing the report
...
1. Go to the `Partner Center` → `Customize` → `Markets` tab
...
## What can't I customize? {#what-you-cant-customize}
...
## Do template changes apply to existing reports? {#template-changes-are-not-retroactive}
...
## Can partners on the Free tier customize the Snapshot Report? {#customization-for-partners-on-free}
...
## How do I configure advanced customization? {#how-to-configure-advanced-customization}
...
For a single report: open the report in **Edit Report** mode and uncheck the box beside any section you do not want the client to see. For all accounts: Navigate to `Partner Center` → `Customize` → `Partner Defaults` → `Sales` → `Edit Default Snapshot Template` and toggle off unwanted sections. For specific markets: Go to `Partner Center` → `Customize` → `Markets` → select the market → `Sales` → `Edit Default Snapshot Template` and check boxes to remove sections.
</details>
...
<details>
<summary>Why does the Need Assistance? section show a salesperson instead of my default contact?</summary>

By default, the Need Assistance? section shows the assigned salesperson's contact information. If a salesperson is assigned to the prospect's account, their details override the default contact you set in `Administration` → `Customize` → `Sales` → `Edit Default Snapshot Template`.
</details>

<details>
<summary>Can I change the colors in my Snapshot Report header?</summary>

Yes. In the custom header HTML, find `div class="call-to-action" style="background: #3FAAE0;"` and replace `#3FAAE0` with your preferred hex color code. Use in-line or scoped CSS, and preview your changes before saving.
</details>

## Screenshots or videos

### Configuration interface
The customization interface controls report appearance and content.
