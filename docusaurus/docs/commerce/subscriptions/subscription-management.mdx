---
title: "How do I set up and manage subscription billing?"
sidebar_label: Subscription Management
description: "Set up retail subscriptions and subscription billing in Commerce, then edit prices, cancel subscriptions, and add or remove products on client accounts."
sidebar_position: 2
...
content_type: how-to
kb_id: KB-00042
answer_snippet: "Subscription management in Partner Center Commerce creates retail subscriptions when you activate products and bills your clients automatically on a recurring schedule. You can edit prices, cancel subscriptions, and add or remove products in Commerce → Subscriptions."
doc_owner: commerce
...
Subscription management in Partner Center Commerce creates retail subscriptions when you activate products and bills your clients automatically on a recurring schedule. You can edit prices, cancel subscriptions, and add or remove products in `Commerce` → `Subscriptions`.
...
Subscription billing gives you the following:

### Key benefits

- **Automated Invoicing**: Recurring billing generates customer invoices from orders
...
- **Personalized Invoice Scheduling**: Customize payment terms, due dates, payment methods, and memo notes on invoices
- **Financial Visibility**: View costs and retail revenue per client account in one place
- **Subscription Control**: View, update, or cancel client subscriptions
- **Preview Next Invoice**: See upcoming client invoices before they're sent

## How do I set up subscription billing? {#how-to-set-up-subscription-billing}
...
## How do I manage existing subscriptions? {#managing-existing-subscriptions}
...
### How do I edit subscription prices? {#how-to-edit-subscription-prices}
...
### How do I cancel subscriptions? {#how-to-cancel-subscriptions}
...
### How do I add products to subscriptions? {#how-to-add-products-to-subscriptions}
...
### How do I remove products from subscriptions? {#how-to-remove-products-from-subscriptions}
...
<details>
<summary>If I sell a package, does my client get one invoice or several?</summary>

Items in a package that share a billing frequency are billed together, so a package of monthly products produces a single monthly charge for the package total rather than one charge per product.

Packages support mixed frequencies, and items on different cycles bill on their own cycles. A package containing monthly products and a yearly product produces the monthly charge every month and the yearly charge once a year. The retail subscription reflects each item's own billing cycle.

The subscription view lists every product separately, which can look like several charges. Check the next invoice amount rather than the product list to see what the client will actually be billed.

If a client has products that were activated on different dates and you want everything to land on one invoice, set `Use a specific renewal date` in your billing settings. See [Getting started with subscriptions and client billing](/getting-started/getting-started-guides/getting-started-with-subscriptions-and-client-billing).
...
All partners on the Vendasta platform can use retail subscriptions to manage their customer billing whether or not they have merchant services enabled. You can track recurring revenue on every account you sell to.
...
The "Bill by active" billing option supports subscription billing and all its features. Your customer billing is not impacted, and you get these features:

- Retail subscription prices are displayed
- Retail subscriptions can be managed (edit price, cancel subscription)
- Upcoming invoices can be previewed
- The wholesale subscriptions section provides full cost and revenue context
