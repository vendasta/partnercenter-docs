...
In Autotask, go to **Admin** → **Resources (Human Resources)** → **API User**. Create a dedicated API-only user, selecting the integration vendor during creation.
:::
...
5. In the **API Tracking Identifier** section, choose **Integration Vendor** and select the vendor.
6. Save the user.
...
|---------|-------------|-------------|
| **Sync back to Autotask** | When a Autotask customer's details are updated in the platform, the change is reflected in Autotask | Leave selected for two-way sync |
