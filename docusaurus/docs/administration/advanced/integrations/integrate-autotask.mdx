...
In Autotask, go to **Admin** → **Resources (Human Resources)** → **API User**. Create a dedicated API-only user, selecting the integration vendor (AMP) during creation. You do not need a separate Integration Code — it is tied to the API user through the vendor selection.
:::
...
5. In the **API Tracking Identifier** section, choose **Integration Vendor** and select the vendor (**AMP**). This links the integration code to the API user — you will not need to enter it anywhere.
6. Save the user.
...
You do not need to enter an Integration Code — it is tied to the API user through the integration vendor (AMP) selected during API user creation.
