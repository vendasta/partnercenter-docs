---
title: "How do I connect Vendasta to Zapier?"
sidebar_label: Zapier Integration
description: "Connect Vendasta to Zapier, set up triggers and actions, start Zaps from Automations webhooks, and fix common Zapier integration issues."
...
content_type: how-to
kb_id: KB-00046
answer_snippet: "Vendasta's Zapier integration connects your Vendasta account to thousands of other apps. Connect it from Administration → Integrations, then use the Vendasta app for Zap actions, the Business App app for triggers, or Webhooks by Zapier to start Zaps from Vendasta Automations."
...
Vendasta's Zapier integration connects your Vendasta account to thousands of other apps. Connect it from `Administration` → `Integrations`, then use the `Vendasta` app for Zap actions, the `Business App` app for triggers, or `Webhooks by Zapier` to start Zaps from Vendasta Automations.

This integration lets you automate workflows between Vendasta and other applications you use daily.
...
- Create workflows without custom coding
...
## What can you do with the Zapier integration? {#what-you-can-do-with-zapier-integration}
...
## How do I set up the Zapier integration? {#how-to-set-up-zapier-integration}
...
## Which Zapier triggers and actions are available? {#available-triggers-and-actions}
...
## How do I trigger Zaps from Vendasta Automations? {#how-to-trigger-zapier-zaps-using-vendasta-automations}
...
## What should I know about the Zapier app? {#zapier-app-information-and-faq}

### What is Beta mode?

The Beta tag on the Zapier integration indicates that features are still being added to it.

:::note Subscription Requirement
This feature is available only to Partners on a subscription tier that includes integrations.
:::

### Who can use the app?

The app can be used by Partners, Salespeople, and their clients as it uses the permissions of the user that set up the Zap. The app is Vendasta branded, so if you white-label the platform, you can't offer this app to your clients. You may instead choose to build the zaps on behalf of your clients.
...
The app can be used by Partners, Salespeople, and their clients. It uses the permissions of the user who set up the Zap. Note that the app is Vendasta branded, so white-labeling options are limited.
</details>
...
<details>
<summary>How do I get a webhook URL to trigger a Zap from Vendasta Automations?</summary>

In Zapier, create a Zap, select `Webhooks by Zapier` as the trigger app, and choose `Catch Hook` as the trigger event. Copy the webhook URL that Zapier shows, then paste it into a `Webhook` action in your Vendasta Automation and save the Automation.
</details>

<details>
<summary>Can I sync contacts from an external CRM like HubSpot or Salesforce into the Vendasta CRM?</summary>

Yes. Build a Zap that uses a new contact in your external CRM as the trigger and the Vendasta `Create or Update CRM Contact` action. The Vendasta app can also create or update CRM companies and log CRM activities.
</details>

<details>
<summary>Where do I find the Zapier integration in Partner Center?</summary>

Go to `Administration` → `Integrations`, locate the Zapier integration tile, and click `Connect`. You're directed to Zapier, where you can start creating Zaps.
</details>