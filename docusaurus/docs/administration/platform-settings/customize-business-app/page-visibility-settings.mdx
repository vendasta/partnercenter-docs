...
Navigate to `Partner Center` → `Administration` → `Customize Business App` → `Pages`, then select the page you want to configure.

## Understanding `Show this page`

Most pages have a `Show this page` checkbox that controls whether the page appears in customers' Business App navigation. Tick it to show the page, clear it to hide the page.
...
### Should I show the Next steps card?

The `"Next steps" card` (`showGetStarted`) setting controls the checklist of setup tasks on Home — connecting Google, adding web chat, registering a texting number, and so on. Items clear as each task is completed or dismissed. See [the AI onboarding experience](/business-app/ai-onboarding-experience#next-steps-cards) for what the cards contain.

**When to show:** You want customers guided through setup inside Business App
**When to hide:** You onboard customers through other channels, such as calls or custom training

The Home page also carries the `"AI has done the work" card` and `"Recommended for your business" cards` settings — both covered in [the AI onboarding experience](/business-app/ai-onboarding-experience#how-partners-customize-what-smbs-see).

---

## Where the Get Started settings went

`Pages` no longer has a `Get Started` entry — opening that address takes you to `Pages` → `Home` instead.

Onboarding guidance for new customers is now the **Next steps** card on Home, controlled by the `"Next steps" card` checkbox described above. That checkbox uses the same `showGetStarted` setting as the old page, so whatever you had configured carries over.

:::info
The custom onboarding video and Customer Journey settings that lived on the old Get Started tab are no longer configurable from Customize Business App.
:::
...
1. Go to `Partner Center` → `Administration` → `Customize Business App` → `Pages` → `Inbox/Messages`
