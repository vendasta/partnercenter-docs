---
title: "How do I configure notifications and onboarding campaigns in Business App?"
...
content_type: how-to
kb_id: KB-00141
answer_snippet: "The Notifications settings in Customize Business App in Partner Center control whether your clients receive Business App notifications, show a global announcement banner, and automatically start an onboarding campaign when a client first logs in."
...
The Notifications settings in `Customize Business App` in Partner Center control whether your clients receive Business App notifications, show a global announcement banner, and automatically start an onboarding campaign when a client first logs in.

![Notifications settings in Customize Business App](./img/notifications-settings.png)
...
<details>
<summary>Can I turn off notifications for all Business App users?</summary>

Yes. Clear the `Allow sending of notifications` checkbox in `Customize Business App` → `Notifications` and click `Save` to stop all notifications for Business App users. To turn off only specific notification types, use `Administration` → `Customize Design` → `Default Notifications` instead.

</details>

<details>
<summary>How do I turn on the automatic onboarding email?</summary>

Create an onboarding campaign at `Partner Center` → `Marketing` → `Campaigns` → `Product Adoption`. Then go to `Customize Business App` → `Notifications`, enable the `Onboarding campaign` checkbox in the `New customer experience` section, select your campaign, and click `Save`.
