...
The users are added to the account, and each receives the [onboarding email](/business-app/ai-onboarding-experience) — inviting is what sends it. If someone isn't ready for Business App yet, keep them as a contact in the CRM and invite them when the time is right; the panel will suggest them by name.

### Create a user from Manage Users

1. Navigate to `Accounts` → `Manage Users`.
2. Click **Create user**.
3. Fill in the user information (email address, name, and other required fields). A **Preview onboarding email** button shows the email as the user will receive it, and you can add a custom introductory message above the standard content.
4. Assign the user to the appropriate account.
5. Click **Save** to create the user.

Creating the user sends the [onboarding email](/business-app/ai-onboarding-experience) — there is no separate step or checkbox for it. This workflow is useful when managing users centrally rather than starting from a specific account.
