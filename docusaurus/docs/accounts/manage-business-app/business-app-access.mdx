...
sidebar_position: 2
description: "Find a client's Business App URL, invite users, review sign-in options and product permissions, and troubleshoot Business App access issues."
...
kb_id: KB-00128
answer_snippet: "Business App access is how your clients log in to their Business App dashboard to view their data and reports and manage their account. Each account has a unique Business App URL, and you invite users from Partner Center so they can set their own password."
...
Business App access is how your clients log in to their Business App dashboard to view their data and reports and manage their account. Each account has a unique Business App URL, and you invite users from Partner Center so they can set their own password.

## What is business app access?

Business App Access refers to the customer-facing dashboard and portal where your business clients can log in to view their data, reports, and manage their account settings. Each customer account has a unique Business App URL and login system that gives them secure access to their business data.
...
## How do I set up Business App access? {#how-to-set-up-business-app-access}
...
1. **Invite users** from the account — the [Invite users panel](/accounts/manage-users) sends each person an [onboarding email](/business-app/ai-onboarding-experience) with a secure set-password link
2. **Provide Business App URL** and login instructions
3. **Offer initial training** or orientation session
4. **Follow up** to ensure successful access and usage

**If a user needs a fresh set-password link:**
1. Open the **options menu (⋮)** next to the user — on the account or in **Accounts → Manage Users**
2. Choose **Send Onboarding Email** or **Send Set Password email (Welcome email)**
3. **Follow up** to confirm successful login and initial setup

Passwords are always set by the user through the emailed link — there are no temporary passwords to share.
...
## How do I configure Business App features? {#how-to-configure-business-app-features}
...
## How do I support customer access? {#how-to-support-customer-access}
...
## How do I manage multiple users per account? {#managing-multiple-users-per-account}

### Multi-user access setup

**Adding Team Members:**
1. **Open the account and click Invite users** — type each team member's email in the [Invite users panel](/accounts/manage-users) and send the batch; each person receives the onboarding email with a set-password link
...
3. **Coordinate initial training** for the team
...
## How do I invite team members to Business App? {#how-to-invite-team-members-to-business-app}
...
## How do I restrict a user to specific products? {#how-to-restrict-a-user-to-specific-products}
...
## How do I enable notifications for a user? {#how-to-enable-notifications-for-a-user}
...
From **Partner Center**, you can invite users to any account with the [Invite users panel](/accounts/manage-users). From **within Business App**, only **Admin users** or existing **Business App users** associated with the account can send invitations — partners logging in as the user (impersonation) cannot send invitations from inside Business App; use the Invite users panel in Partner Center instead.
</details>
