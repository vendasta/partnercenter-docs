...
A [field layout](/administration/data-management/crm-objects/field-layouts) — the grouping and ordering of fields on contact, company, opportunity, and custom object records — can be rolled out with a template so every account presents its records the same way.
...
- Applying a template replaces the target account's layout for that object type. Any layout customization already made on the target account is overwritten.
