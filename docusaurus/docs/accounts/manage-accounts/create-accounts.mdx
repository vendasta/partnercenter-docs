...
- Required and recommended field configuration
...
3. **Fill in the recommended business information**:
   - `Street Address`, `City, State/Province, ZIP`, and `Country/Region`: Complete location details
...
`Business Name` is the only field required to create the account. The fields above are recommended rather than required — if you leave any of them blank and click `Create Account`, a warning lets you choose to continue or go back and complete them. Going back highlights the incomplete recommended fields.
...
6. Click `Create Account` to save
...
2. Fill in the business name and, if available, a street address. If the SAB has no public address, use the owner's address or a city/ZIP placeholder.
...
2. Enter the business name and, if available, a street address (use the owner's address or a placeholder city/ZIP if there's no public address).
...
Yes. When creating an individual account, the **Users** panel lets you add the account's first users during creation — they receive the [onboarding email](/business-app/ai-onboarding-experience) once the account and its products are ready. For bulk imports, you can configure default user settings, including whether the welcome email is sent.
</details>
...
<details>
<summary>Can I create an account without a phone number, address, category, or website?</summary>

Yes. `Business Name` is the only required field when creating an account manually. Leaving the others blank shows a warning when you click `Create Account`, but doesn't block you from creating the account. Keep in mind that missing information — like a street address — can limit which products work correctly until it's added later.
</details>
