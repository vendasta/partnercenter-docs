---
title: "How do I create accounts in Partner Center?"
sidebar_label: Create Accounts
sidebar_position: 2
description: "Create business accounts in Partner Center one at a time with business search or manual entry, or in bulk with a CSV import."
...
content_type: how-to
kb_id: KB-00178
answer_snippet: "To create an account in Partner Center, go to Accounts → Manage Accounts and click Create Account to search for the business or enter it manually. To create many accounts at once, click Import Accounts and upload a CSV file."
...
To create an account in Partner Center, go to `Accounts` → `Manage Accounts` and click `Create Account` to search for the business or enter it manually. To create many accounts at once, click `Import Accounts` and upload a CSV file.
...
## How do I create an individual account? {#how-to-create-individual-accounts}
...
## How do I import multiple accounts? {#how-to-import-multiple-accounts}
...
3. **Configure import settings**:
   ...
   - Default email language for the onboarding email (every imported user receives it)
...
## How do I handle account creation issues? {#how-to-handle-account-creation-issues}
...
## Which user and notification settings can I set during a bulk import? {#bulk-import-settings-users-and-notifications}
...
Every user created by the import is sent the [onboarding email](/business-app/ai-onboarding-experience); the wizard states this rather than offering a choice. Alongside the confirmation you get a **Preview** link, a **Default email language** for recipients whose language isn’t set, and an optional custom introductory message applied to the whole import.

:::info
You can adjust user permissions later from Manage Users if needed.
:::

## Why doesn't a business appear in the account search? {#troubleshooting-business-search}
...
Yes. When creating an individual account, the **Users** panel lets you add the account's first users during creation — they receive the [onboarding email](/business-app/ai-onboarding-experience) once the account and its products are ready. For bulk imports, every user created by the import receives the onboarding email; you can set a default email language, but you can't turn the email off.
</details>
...
<details>
<summary>How do I create multiple accounts at once?</summary>

Go to `Accounts` → `Manage Accounts`, click `Import Accounts`, and download the CSV template. Fill in the required columns, save the file as CSV UTF-8, and upload it. Automatic product activation doesn't run on CSV-uploaded accounts, so activate products separately after the import.
</details>
