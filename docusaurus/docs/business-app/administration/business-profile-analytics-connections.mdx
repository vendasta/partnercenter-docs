...
Business name is the only field required to save a Business Profile. Category, phone number, website, and address are recommended rather than required, so an account can save its profile without them.

When one of those recommended fields is left blank, Business App shows a warning at save time so the account holder can choose to continue saving or go back and complete it. Going back highlights the incomplete recommended fields.

If a recommended field already had saved information and that information is removed, the field becomes required again and the profile can't be saved until it's filled back in — this prevents an account from accidentally clearing details that an active product or service depends on.
...
<details>
<summary>Can an account save its Business Profile without a phone number, address, category, or website?</summary>

Yes. Business name is the only required field. The others are recommended — Business App shows a warning before saving if they're incomplete, but doesn't block the save. If one of those fields already had data and it's cleared, it becomes required again until it's filled back in.
</details>
