...
7. Confirm the scheduling success notification and where the post appears next (`Scheduled`)
8. Open `Insights` > `Post performance` to show engagement metrics

:::info Scheduling from draft post cards
- The `Schedule` action appears on draft post cards.
- `Schedule` is available only for posts in the draft state.
- The `Schedule` action is available in both desktop and mobile toggle views.
- If the selected date and time is in the past, scheduling is unavailable and the UI shows: `Please select a future date and time to schedule this post`.
- If the selected date and time is in the past and no social account is connected, the UI shows: `Please ensure the scheduled time is in the future and at least one social account is connected`.
- After scheduling succeeds, a confirmation snack bar appears.
:::
