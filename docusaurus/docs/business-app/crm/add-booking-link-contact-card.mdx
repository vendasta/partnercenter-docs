...
Users using Business App can display a CalendarHero meeting link (or any meeting scheduling links) on their salesperson profile, which will then appear inside the Business App contact card for accounts that have been assigned to that salesperson user.
