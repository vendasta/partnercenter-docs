...
Campaign Pro does not have SMS capabilities included in the base product without any additional add-on. To send SMS campaigns using Campaign Pro, customers must purchase an SMS add-on product. The base Campaign Pro plan only includes email campaigns (e.g., 10,000 emails monthly) and does not include SMS sending credits.

**Key points:**

- Campaign Pro automatically comes with SMS capabilities technically, but to utilize SMS sending, SMS credits must be purchased separately as an add-on.
- Without the SMS add-on, the SMS campaign feature remains locked and attempts to use it prompt the user to purchase the SMS add-on.
- SMS add-ons are available for purchase and provide a set number of SMS credits (e.g., 100 or 500 SMS messages per month depending on the add-on).
- The SMS add-on is distinct from other SMS credits that might be purchased for other products like Reputation Management.
- Inbox Pro can be used to respond to SMS replies if activated, but sending bulk SMS campaigns requires the SMS add-on for Campaign Pro.

Therefore, SMS capabilities are not included by default with Campaign Pro and require purchasing an additional SMS add-on product to enable SMS campaign functionality.
...
For more information on digital marketing compliance, please ensure you follow local regulations and carrier requirements for SMS messaging.