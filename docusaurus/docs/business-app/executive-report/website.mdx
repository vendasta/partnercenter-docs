...
sidebar_position: 10
description: Monitor website performance through WordPress Hosting presence and Google Analytics metrics in the Executive Report.
brand: vendasta
...
If you have a Google Analytics account connected to Business App, you'll find the Executive Report even more useful for monitoring your website's performance. You can see improved measurements of your site's traffic, referral sources, bounce rate, and more.

## How website performance reporting works

### WordPress Hosting integration
Website performance information will appear in the Executive Report when you activate WordPress Hosting (Standard, Premium, or Pro) for an account and connect a WordPress site.

![Website Performance in Executive Report](./img/website-performance.jpg)

### Google Analytics integration

You can enhance your website reporting by connecting a Google Analytics account. A few important things to know before connecting:

- **WordPress Hosting is not required**: any account with a GA4 property can connect Google Analytics to the Executive Report.
- **GA4 is the supported version**: Universal Analytics (UA) is no longer supported by Google. Connect a GA4 property.
...
Once connected, data collected from Google Analytics will be displayed under the Website section of the Executive Report. If no data appears after 48 hours, verify the connection is still active under `Business App` → `Administration` → `Integrations`.
...
1. **For WordPress Hosting data**: Activate WordPress Hosting (Standard, Premium, or Pro) for the account and connect a WordPress site
2. **For enhanced Google Analytics data**: Connect a Google Analytics account through `Business App` → `Administration` → `Integrations` → `Browse Integrations`
...
Small discrepancies (1-5%) are normal. If the discrepancy is large (over 10-15%), check whether the correct GA4 property is connected and whether the connection is still active under `Business App` → `Administration` → `Integrations`.