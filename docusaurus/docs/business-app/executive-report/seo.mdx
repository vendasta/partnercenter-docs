...
- In `Business App` → `Administration` → `Integrations` → `Browse Integrations`, you'll find a card to connect to Google Search Console.
...
- When Google Search Console is first connected, Business App will automatically pull in this business's top 10 search queries by click volume for reporting.
- You can customize these keywords, remove them, or add, up to 20 keywords to be reported in the Executive Report.
- Go to `Business App` → `Administration` → `Integrations` and click on the `Google Search Console` card to edit the search queries.
...
If you have completed these checks and the issue persists, contact Vendasta Support for further investigation.

## FAQs

**How do I put this Search Console data at the top of my Executive Reports?**

- You can customize the order of the sections for the single-location Executive Report, from `Partner Center` → `Accounts` → `Manage Business App` → `Customize Business App` → `Executive Report` → `Sections`. From here, you can reorder sections by dragging the SEO section to the top of your report, and hide/show sections based on your service focus. This can be configured on a market-by-market basis. See our [Executive Report Customization Guide](/accounts/reports/customize-executive-report) for detailed instructions.
