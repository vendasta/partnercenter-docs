...
Everyone in the batch receives the onboarding email with the default configuration unless customized — the panel notes this up front. There's no per-user email opt-out in the panel; to add a user without sending any email, use **Accounts → Manage Users → Create user** and clear the email checkbox there.
