...
- **Credits and Usage:** There have been no changes to the credits system. Conversations AI remains unlimited, and Reputation AI Premium still uses credits as before.
...
Usually no. The registered business name shows in the app UI and is not customer-facing today, so a name change alone does not require a new A2P registration. If you still need the registered name updated, open a Twilio support request (it can take up to four weeks), then ask Vendasta support to complete the internal update.
</details>

<details>
<summary><strong>Which Business Type should I choose for a sole proprietor in SMS registration?</strong></summary>

Sole proprietorships are not supported for SMS registration in Partner Center. That registration path is not available yet, so there is no Business Type that completes the flow for a sole proprietor.
</details>