...
<details>
<summary>My client's website chatbot looks quiet in Conversations even though the site has traffic. What should I check?</summary>

Open `Conversations` and review the **Anonymous Visitors** set. Anonymous conversations are hidden by default. Chats stay anonymous when the visitor never provides lead information (and the assistant never requested it), so low lead volume is often a prompt or lead-capture setup issue rather than a broken widget.
