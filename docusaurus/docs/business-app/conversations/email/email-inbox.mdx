...
kb_id: KB-00077
answer_snippet: "Conversations AI lets your client's team send and receive email with leads and customers from one shared address in the format reply+xxxxx@businessapp.io. Customer replies return to the same conversation, and email forwarding brings mail sent to your client's existing address into Conversations."
...
Conversations AI lets your client's team send and receive email with leads and customers from one shared address in the format `reply+xxxxx@businessapp.io`. Customer replies return to the same conversation, and email forwarding brings mail sent to your client's existing address into Conversations.
...
- Available on Conversations AI Standard, Pro, and Premium plans
...
### How do I send an email? {#sending-emails}
...
To view a single email on its own, select `View` on that message in the conversation. Selecting the back arrow returns your client's team to the main conversation.
...
### How do I set up forwarding? {#how-to-set-up-forwarding}
...
### How do I forward from Gmail? {#forward-from-gmail}
...
### How do I forward from Outlook? {#forward-from-outlook}
...
<details>
<summary><strong>Is the shared Conversations address the same as the reply address in Email Configuration?</strong></summary>

No. The Conversations address is the `Forwarding address` in `Business App` → `Administration` → `Email Configuration` → `Advanced email settings`, and it looks like `reply+xxxxx@businessapp.io`. The `Sender and reply settings` section applies to Campaigns and other integrated services, not Conversations.
</details>

<details>
<summary><strong>Which Conversations AI plans include email?</strong></summary>

Email is available on Conversations AI Standard, Pro, and Premium plans.
</details>

<details>
<summary><strong>Can the AI respond to emails?</strong></summary>

Yes. Conversations AI supports AI auto-responses for 24/7 customer support via email. To have emails sent to your client's existing address reach Conversations, set up email forwarding to the Conversations forwarding address.
</details>

<details>
<summary><strong>After forwarding is set up, will emails still arrive in my client's Gmail or Outlook inbox?</strong></summary>

Yes. Forwarding sends a copy of each email to Conversations, where it appears as a new conversation. Your client's team can then read and reply from Conversations.
</details>
