...
1. Turn the billing toggle **On**.
2. Choose a **Collection method**:
   - **Create a draft invoice**: generates an invoice and saves it as a draft for you to review and send manually
   - **Email payable invoice to billing recipient**: automatically emails the invoice to your client on renewal
   - **Automatically charge payment method on file**: charges your client's saved card automatically (requires Vendasta Payments and a saved payment method)
3. Set the **Collect on** day to control when invoices are generated each month.
4. Click **Save**.
