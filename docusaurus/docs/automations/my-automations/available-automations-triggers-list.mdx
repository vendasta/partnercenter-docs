...
Some triggers are simple and are ready out-of-the-box, while other triggers may require specifying **trigger options**. Learn more about triggers [here](./what-are-automation-triggers.mdx).
...
| Triggered via Zapier | Starts the automation when the "Run Automation" action is used in our Zapier app. | [Trigger an automation using Zapier.](/business-app/automations/trigger-automation-using-zapier.mdx) | I want to make an automation that I can trigger from Zapier, to update contacts in the CRM, and send along the data as Zapier provides it. |
### Contacts
