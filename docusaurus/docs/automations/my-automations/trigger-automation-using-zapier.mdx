---
title: "How do I trigger an automation using Zapier?"
sidebar_label: Trigger via Zapier
description: "Connect the Vendasta CRM to third-party apps through Zapier and use the Run Automation action to trigger your Vendasta automations."
sidebar_position: 6
...
content_type: how-to
kb_id: KB-00034
answer_snippet: "You can trigger a Vendasta automation from any third-party app in Zapier by using that app as the Zap trigger and the Vendasta App's Run Automation action. The automation must be turned on in Vendasta and use a compatible trigger."
...
You can trigger a Vendasta automation from any third-party app in Zapier by using that app as the Zap trigger and the Vendasta App's `Run Automation` action. The automation must be turned on in Vendasta and use a compatible trigger.

The Vendasta App in Zapier lets you connect the Vendasta CRM to third-party systems. You can select any third-party system available in Zapier as a trigger that initiates an action in the Vendasta CRM.

## How do I trigger an automation with Zapier? {#how-triggering-an-automation-with-zapier-works}
...
**Step 2: Choosing the Action App**
...
## How do I connect Facebook Lead Ads to Vendasta automations? {#connect-facebook-lead-ads-to-vendasta-automations}
...
<details>
<summary>Why don't I see my automation in the Automation ID list?</summary>

Make sure the automation uses a compatible trigger, such as Triggered via API or Triggered manually for an Account or Order, and that it is turned on in Vendasta. Automations that are off can't be triggered via Zapier.
...
Enter your Partner ID, or the Account ID of the client you are setting up Zapier for. The Organization ID field is then populated automatically based on the ID you signed in with.

</details>

<details>
<summary>Can I trigger a Vendasta automation from QuickBooks?</summary>

Yes. In Zapier, set a QuickBooks Online event such as New Payment as the trigger, then add the Vendasta App's Run Automation action and select the automation to run.

</details>

<details>
<summary>Can I sync Facebook Lead Ads leads with the Vendasta CRM?</summary>

Yes, through Zapier. Set **Facebook Lead Ads** with **New Lead** as the trigger, then add a Vendasta action: **Run Automation** to run a Vendasta automation for each lead, or **Create or Update Contact** to write the lead directly to the CRM.

</details>

<details>
<summary>How do I make sure incoming data maps to the right CRM fields?</summary>

Map the fields in the Vendasta action step in Zapier, for example first name, last name, email, and phone to the corresponding Vendasta CRM fields. Test the action before you publish the Zap to catch missing fields or incorrect data.

</details>

<details>
<summary>How do I link a contact to a company from Zapier?</summary>

Pass the numeric Company ID, not the company name, in the **Primary Company** field. The company must already exist in the CRM, and you can find its ID in the company's URL in `Partner Center` → `CRM` → `Companies`.

</details>

<details>
<summary>How do I know my Zap is working?</summary>

Test the step in Zapier before publishing. A 'Run Automation Successful' message confirms the Zap is working and ready to publish.
