...
Once saved, the reference is stored as a step ID — for example `{.WorkflowStep.773dcc25-db4c-4d5d-882d-d3c8482e9770.response}` — which doesn't tell you which earlier step it points to. To see those connections on the canvas, turn on [Step references](./advanced-automation-features.mdx#step-references) in the automation editor toolbar.
...
3. **Use descriptive step names** — Clear step names, plus [Step references](./advanced-automation-features.mdx#step-references) on the canvas, make it easy to see where each expression's data comes from
