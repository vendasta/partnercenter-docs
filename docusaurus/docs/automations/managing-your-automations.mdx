...
- **Trigger Details**: See exactly what event initiated the automation (e.g., which specific contact was created or which form was submitted).
- **Step-by-Step Progress**: Drill into specific runs to see detailed step-by-step progress, including which steps completed successfully and which failed.
- **Data Changes**: What data was modified by each step.
- **Error Logs**: Get precise details on what caused errors and where in the workflow they happened.
