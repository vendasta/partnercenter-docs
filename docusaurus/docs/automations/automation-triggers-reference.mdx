...
| An invoice status is changed | Starts the workflow when an invoice status changes (e.g., to Past Due or Paid). | This trigger is ideal for automated reminders but cannot dynamically include invoice PDFs or payment links. | When an invoice becomes past due, send a reminder email and create a follow-up task. |
...
6. **Billing Follow-ups**: Use "invoice status changed" for dunning and reminders.
