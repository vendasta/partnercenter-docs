...
| Step | Overview | Special Cases | Example Use Cases |
| --- | --- | --- | --- |
| Send a request to an AI employee | Send a prompt to a selected AI employee. | | When a company is created, prompt the AI agent to research the company, and create a summary note. |
| Send a prompt to AI | Send a custom prompt to AI and receive a response. Supports template variables for account, order, and partner context. | Prompt is required and must be under 4000 characters. | When a new contact is created, prompt AI to generate a welcome SMS based on contact info. |
