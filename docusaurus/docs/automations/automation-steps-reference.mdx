...
description: Complete reference guide for all available automation steps and actions your automations can perform in Vendasta's platform.
sidebar_position: 4
---
...
This article provides a complete reference of all automation steps that can accompany each trigger. We have video examples of how to configure these steps [at the bottom of this page](#walkthroughs).

## Automation Steps by Category

### Accounts & Users

| Step | Overview | Special Cases | Example Use Cases |
| --- | --- | --- | --- |
| Set account lifecycle stage* | Sets an account's Lifecycle stage to a specific stage, i.e.: Lead, Prospect, Customer | A valid lifecycle stage (Lead, Prospect, or Customer) must be selected. The account must exist. | When an opportunity is created on an account, set its Lifecycle stage to Prospect |

### AI

| Step | Overview | Special Cases | Example Use Cases |
| --- | --- | --- | --- |
| Send a request to an AI employee | Send a prompt to a selected AI employee. | An AI employee must be selected. The prompt is rendered with template variables before being sent — if it renders to empty, an error occurs. | When a company is created, prompt the AI agent to research the company, and create a summary note. |
...
| Get associated Contact from Company | Gets the Contact associated with a CRM company. | If one does not exist, an error is returned. | When a company is created, I would like to get the associated contact and begin an email campaign to them. |
| Get associated Company from Contact | Gets the Contact associated with a CRM contact. | If one does not exist, an error is returned. | When a contact is created, I would like to get the associated company and notify the assigned salesperson on the company. |
| Get associated Contact from Opportunity | Gets the Contact associated with a CRM opportunity. | If one does not exist, an error is returned. | When an opportunity is created, I would like to get the associated contact and begin an email campaign to them. |
...
| Step | Overview | Special Cases | Example Use Cases |
| --- | --- | --- | --- |
| Assign a sales team | Assigns a random salesperson from the indicated sales team to the account. If the account already has a salesperson, they can optionally be replaced. | If reassign is not enabled, accounts with an existing salesperson are skipped. Fails if the team has no members. | When a trial product is activated, assign a salesperson from the team responsible for that product. |
| Assign a salesperson | Assigns a specific salesperson to the account. Multiple individuals from across multiple teams can be selected, in which case, one will be randomly assigned. If the account already has a salesperson, they can optionally be replaced. | If reassign is not enabled, accounts with an existing salesperson are skipped. Fails if no matching salesperson is found. | When a highly technical product is activated, assign a highly knowledgeable salesperson. |
...
| Step | Overview | Special Cases | Example Use Cases |
| --- | --- | --- | --- |
| Apply Account Template | Deploys an account template to apply AI assistants, CRM fields, automations, etc. to the target account. Runs asynchronously. | An account template must be selected. Supports data passing for dynamic template selection. Existing objects are handled per type (skipped or updated). | When a partner creates an account via API for a new dental client, automatically apply the "Dental Onboarding" template to configure their AI assistant and CRM fields. |

## Step Configuration Best Practices

### Choosing the Right Steps

1. **Plan Your Workflow**: Map out the entire customer journey before selecting steps
2. **Consider Dependencies**: Ensure steps are in the right order and have necessary prerequisites
3. **Handle Errors**: Plan for what happens when steps fail
4. **Test Thoroughly**: Test each step individually and as part of the complete workflow

### Common Step Patterns

1. **Welcome Workflows**: Account creation → Assign salesperson → Send welcome campaign → Create follow-up task
2. **Product Onboarding**: Product activation → Send education campaign → Create fulfillment project → Schedule check-in
3. **Lead Nurturing**: Contact creation → Add to list → Send campaign series → Log activity based on engagement
4. **Payment Processing**: Payment failure → Notify team → Add to overdue list → Create follow-up task
5. **Opportunity Management**: Opportunity creation → Assign salesperson → Set lifecycle stage → Create tasks

### Performance Considerations

1. **Minimize API Calls**: Combine related actions where possible
2. **Use Appropriate Delays**: Don't make workflows longer than necessary
3. **Batch Operations**: Group similar actions together
4. **Monitor Resource Usage**: Be aware of system limits and quotas
5. **Optimize Conditions**: Use efficient filtering to reduce unnecessary step execution
...
- [Automations overview](./index.mdx) - Basic concepts and first steps
- [Creating and Configuring Automations](./creating-and-configuring-automations.mdx) - Configuration and settings
