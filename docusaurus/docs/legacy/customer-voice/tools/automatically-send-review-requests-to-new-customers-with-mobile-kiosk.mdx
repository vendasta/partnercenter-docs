...
3. Paste the URL into any browser on any device, then bookmark the page

:::note
The Mobile Kiosk sends the default template to the new customer. Change your default template at `Templates` → `Make Default Template`.
:::
...
:::note
When the default template is an email template, only first name and email are required. When it is an SMS template, only first name and mobile number are required.
:::

3. The customer must check the box to specify they consent to provide their contact information to the business, then click `Submit`

4. A confirmation message will appear, notifying the customer that a review request has been sent to their email or phone number, then click `Done`.
