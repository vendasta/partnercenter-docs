...
**On their side, in Business App.** Set up each of these on the client's account:

- Their branding
- The AI Receptionist's hours, greeting, and lead-capture questions
- The chat widget on their website
- Review sources connected
- Social accounts connected, with the first posts scheduled
- Any CRM or scheduling tool they already run

Each of those is a procedure in its own right. [Deploy a receptionist for a client](/learn/ai-workforce/deploy-a-receptionist-for-a-client) takes you through the receptionist and the widget on a client's account, and it picks up from [Bring in your first client](/learn/getting-started/bring-in-your-first-client) and [Get set up](/learn/getting-started/turn-on-your-own-ai-receptionist). Finish your side before you start theirs.

Configured is not the same as working. Setup is complete when every component has passed a live test, and running it takes ten minutes. Check the client's editions first: two of the three tests below only run on the right one, so a failure may be a missing product rather than a broken setup.
...
<LabChecklist
  ...
  confirm={<>The receptionist answers a call, holds a chat, and follows up on a missed call, live on their account, before the client has seen any of it.</>}
/>
</LabCallout>

A client who also bought Reputation AI or Social AI has a review request to send and a first scheduled post to confirm, both set up in [the AI Reputation Specialist setup guide](https://docs.businessapp.io/business-app/ai/ai-workforce/ai-reputation-specialist) and [the Social Media Manager setup guide](/ai/ai-workforce/social-media-manager).

Describe that test run on the handoff call.
...
<KeyPoints
  items={[
    ...
    },
    {
      ...
      text: 'Roughly $2,500 in monthly recurring revenue, modelled at $500 per client, running on a rhythm you own.',
    },
  ]}
/>

Those dates are Vendasta's own benchmark for a partner starting out, modelled at $500 in monthly recurring revenue per client. Your pricing and your close rate move every number in it, so aim at the shape and measure the result against your own book. The first sale is the hard one, and everything in this path exists to make it happen.
