...
<IconFlipCardGrid>
  <IconFlipCard gradient="blue" front="Trigger" back="Start: The event that starts the workflow. Its scope decides which steps you can use." />
  <IconFlipCard gradient="green" front="Condition" back="Guard: The guard that decides whether to run on this record. Without it, the trigger fires on everything." />
  <IconFlipCard gradient="purple" front="Action" back="Do: A step the workflow takes: send, create, log, wait. They run top to bottom." />
</IconFlipCardGrid>
