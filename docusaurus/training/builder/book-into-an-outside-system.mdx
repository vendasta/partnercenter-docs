...
That is a different build from connecting a calendar and switching on a booking capability. The rest of this step walks one real integration of this shape, a franchise brand's field service system, because the pattern generalizes: the same questions come up with any scheduling system a client already runs.
...
One capability holds all three calls. The platform is explicit that a capability takes [as many tools as the job needs](/ai/ai-capabilities/creating-custom-capabilities), so the sequence lives in the capability prompt: which call runs first, what each one hands to the next, and what the employee says while it works.
...
:::tip Try it now
Take a system one of your clients already runs and open its API documentation. Find three things: how it authenticates, which endpoint creates a contact, and whether it documents a way to search availability. Anything you cannot find is your first question for that vendor, and it is a faster question to ask than to answer yourself.
:::
