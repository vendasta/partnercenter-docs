...
<LessonHeader
  ...
  required={["Partner Center access", "A CRM with at least one contact source already flowing in, such as a form or an AI Employee"]}
  lab
...
## Build a smart list that triggers automatically

*You are here: Partner Center → `Automations`.* The automation you just built waits for a conversation to end. This one waits for nothing: a list decides who is in it, and the automation runs on whoever lands there.
...
<LabChecklist
  ...
  steps={[
    <>Select <code>Create list</code>, choose <strong>Smart</strong> as the list type, and name it.</>,
    <>Add a filter, for example record source equals form submission.</>,
    <>Select <code>Create</code>.</>,
  ]}
...
<LabChecklist
  ...
  steps={[
    ...
    <>Select <code>Publish</code>, then turn the automation <code>On</code>.</>,
  ]}
...
Go back to `Automations` → `My automations`.
...
<LabChecklist
  ...
  steps={[
    ...
    <>Select <code>Publish</code>, turn the automation <code>On</code>, then submit a test form yourself to confirm it fires.</>,
  ]}
...
## Branch review requests by location
...
Anything that reaches a client's customers on its own, especially content a client did not expect to see posted automatically, deserves a human approval step before it goes out. Build the review in as a step in the automation.
...
<LessonFooter to="/learn/builder/wire-it-to-your-systems" pathName="Wire your AI Workforce to act" step={5} totalSteps={10} />
