...
<LabChecklist
  ...
  steps={[
    ...
    <>Select the <code>Add knowledge</code> field, then <code>+ Add new knowledge</code> at the top of the dropdown, then <strong>File Upload</strong>.</>,
    <>Upload the document, labeled clearly (for example, "2026 Service Pricing").</>,
  ]}
...
<LabChecklist
  ...
  steps={[
    <>Open the capability that answers pricing questions, or select <code>+ Add a capability</code> then <code>+ New capability</code> to write one named something like <code>QuotePricing</code>.</>,
  ...
  questions={[
    {
      ...
      options: [
        ...
        'A channel fix, because the answer went out on the wrong channel'
      ],
      correctIndex: 1,
    ...
    },
    {
      ...
      options: [
        ...
        'Remove the pricing source so it stops answering the question'
      ],
      correctIndex: 2,
...
<LessonFooter to="/learn/ai-workforce/deploy-a-receptionist-for-a-client" pathName="Hire your first AI Employee" step={4} totalSteps={5} />
