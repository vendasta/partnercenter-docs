...
<LessonHeader
  ...
  topics={["AI Receptionist", "Appointment booking", "AI Workforce"]}
...
*You are here: Partner Center, on the same employee you deployed last step.* An employee that only answers is a receptionist who never puts anything on the books. Booking is what turns a good conversation into a kept appointment.

<LabCallout title="connect a calendar">
Go to `CRM` → `My Meetings`, then the `⋮` menu → `Meeting settings` → `Calendar settings`.

<LabChecklist
  storageKey="teach-it-to-book-lab-1"
  steps={[
    <>Select <code>Connect calendar</code>. Each person taking bookings signs in <strong>as themselves</strong>, not through an admin connecting it on their behalf.</>,
    <>Choose <code>Google Calendar</code> or <code>Microsoft 365 / Outlook.com</code>, and grant the requested permissions.</>,
  ]}
...
Your booking URL is set once, under `Meeting settings` → `General meeting URL`. What you build per event type is the duration, the availability window, and the buffer before and after. My Meetings offers durations of 15, 30, 45, or 60 minutes, or a custom length; decide which one your receptionist should offer before you test it.

<LabCallout title="build one booking link">
Stay in `My Meetings` → `Manage booking links` → `Event types` → `Create event type`.
...
## Create a team event that assigns fairly
...
## Enable the Book appointments capability

<LabCallout title="add Book appointments">
Go to your receptionist's `Configure` page → `Capabilities`.

<LabChecklist
  storageKey="teach-it-to-book-lab-3"
  steps={[
    <>Select <code>+ Add a capability</code>, tick <strong>Book appointments</strong>, and select <code>Add</code>.</>,
    <>On the capability card, set <code>Select event link to book with:</code> to the event type you built above.</>,
  ]}
...
If the employee still carries <code>Record preferred booking times (Voice)</code>, which notes a caller's preferred time without booking anything, replace it with Book appointments: a preference is not a kept meeting.
...
Book appointments does not collect payment or hold a slot behind one. A business that wants a deposit first ties sending the link to a payment-confirmation automation instead, rather than expecting the booking capability to gate on it.
...
Go to `Capabilities` → `+ Add a capability` → `+ New capability`.
...
<LabChecklist
  ...
  steps={[
    ...
    <>Inside each branch, reference other capabilities by their exact name, for example "call the Transfer calls capability," rather than restating what that capability already does.</>,
...
Do not build a second after-hours capture from scratch: the Capture leads capability your receptionist already runs after hours can extend to ask which department the caller wanted, the same way any other question gets added to its prompt.
...
Go to `Chat`, or the assigned channel.
...
If a client needs bookings to land in a scheduling system they already run, that is a deeper build, and [this is where to go for it](/learn/builder/book-into-an-outside-system).
  ...
  questions={[
    ...
    },
    {
      ...
      options: [
        ...
        'Reference the Transfer calls capability by its exact name',
        'Ask the caller to call back during business hours',
        'Hand the call to Capture leads instead'
      ],
      correctIndex: 1,
      explanation: 'Referencing another capability by its exact name keeps each capability doing one job, rather than duplicating logic that already exists elsewhere.'
    }
  ]}
/>

<LessonFooter to="/learn/ai-workforce/train-your-employee" pathName="Hire your first AI Employee" step={3} totalSteps={5} />
