...
Everything so far has worked on one account. This is where that account becomes a sale, and a sale becomes a program you manage.

<LessonSection>

## Pitch the outcome, not the feature
...
<LabCallout title="price a real bundle against live rates">
Go to `Marketplace` → `Discover Products`.

<LabChecklist
  storageKey="sell-and-manage-lab-1"
  steps={[
    <>Set <strong>Explore</strong> to <code>All</code>, or clear the <code>Vendor is Vendasta</code> chip. It is applied by default and it hides the service products.</>,
    <>Look up the Conversations AI edition your prospect needs. The columns give you <code>Your cost</code> beside the vendor's suggested retail.</>,
    <>Look up the AI Employee Setup, and the AI Workforce Optimization Plan if you intend to manage it ongoing.</>,
    <>Add the three together and set the monthly retail number you will quote.</>,
    <>Sense-check it against the field packages above, and against what this client currently loses to missed leads.</>,
  ]}
  confirm={<>You have a bundle price built from today's rates rather than a remembered one, for a named prospect.</>}
/>
</LabCallout>
...
## Match the next step to where the client is
...
Review real conversations and send anything off back through the audit habit: a wrong answer is a knowledge gap, a wrong behavior is a capability gap. A deployed workforce that nobody revisits drifts; one reviewed monthly keeps improving on its own trajectory.
