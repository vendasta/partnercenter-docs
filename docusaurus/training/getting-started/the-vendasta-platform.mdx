...
<LessonHeader
  ...
  outcomes={[
    ...
    "Say which view of Partner Center each role on your team gets",
  ]}
/>
...
The chain starts in the **[Marketplace](/marketplace)**: the catalog you shop from, where you pick the products and packages you resell under your own name and your own pricing. Nothing runs for a client until you activate something you chose there.
...
The building blocks do the work; the toolkit around them runs the business. With Vendasta you get the full infrastructure: client management, a catalog of products to resell, automation, billing, and a fulfillment team, all under your own brand. You do not have to use all of it right away. Start where the value lands fastest: one AI Employee running for a client, or a single package that bundles the products a vertical needs, and grow into the rest from there.

Alongside the products you resell, four pieces of that toolkit carry the day-to-day:

**Find what you sell in the [Marketplace](/marketplace).** Browse products and services by category, add the ones you want to your store, and start from a ready-made package rather than building one from scratch.
...
**Partner Center**, at [partners.vendasta.com](https://partners.vendasta.com), is where you run your business and build your own AI workforce; **Business App** is where you build the AI workforce for each of your clients. One question orients you anywhere in the platform: whose workspace am I in?
...
| **People** | Invite your team, set permissions, and fill in salesperson profiles | Invite client users and send the onboarding email once their products are live |
...
Set yourself up once on the left, then repeat the client side of the ritual for every business you bring on.
...
:::tip Start SMS registration early
A client whose AI Employee texts United States numbers registers their business with the carriers first (the registration is called A2P). It happens in their Business App, under `Administration` → `Conversation`, and approval takes anywhere from one week up to a month, so start it as soon as a client plans to text. Rejections usually come down to the website: carriers want a privacy policy and terms of service that cover SMS, and consent language wherever a phone number is collected, so a reviewer can see the customer knowingly agreed. [This example site](https://a2p-example.businessapp.io/) shows a compliant one, consent checkboxes and all. Canadian businesses have nothing to register today; requirements in other markets vary as SMS opens up in them. [This guide covers what the carriers check](/business-app/conversations/sms/us-businesses-sms-registration).
:::

<LessonSection>

## Three roles on your team

One more piece of Partner Center is yours to set: who on your team gets into it, and how much of it they see. Three roles cover everyone. Flip each one:

<IconFlipCardGrid>
  <IconFlipCard
    gradient="blue"
    front="Admin"
    back="The full workspace: accounts, billing, branding, and administration. The right fit for decision-makers, managers, and operations. Running solo, this is you."
  />
  <IconFlipCard
    gradient="green"
    front="Salesperson"
    back="A focused view of Partner Center: the CRM, opportunities, orders, proposals, inbox, and pricing, for working pipeline and talking to clients."
  />
  <IconFlipCard
    gradient="purple"
    front="Digital Agent"
    back="A fulfillment seat. It opens Task Manager rather than Partner Center, so it belongs to the people delivering work rather than selling or running it."
  />
</IconFlipCardGrid>

Permissions go finer than the roles themselves. You can restrict an Admin from specific areas, such as billing reports, platform customization, or creating other Admins. Most people on your team should hold at least some admin access, because permissions can narrow it afterward; defaulting everyone to Salesperson just to limit access gives them less than they need.

Your team lives under `Administration` → `My team`. Each person's row carries their role in the **Role** column, and a banner at the top of the page names how many seats your plan includes and what an additional seat costs per month. `Invite team member` in the upper right asks for a first name, last name, email, and the roles they hold; the roles are checkboxes, so one person can hold more than one. Permissions and market access are set after the person exists, rather than in the invite panel.

[This guide covers every role and permission in full](/getting-started/getting-started-guides/getting-started-with-your-team).
...
<KnowledgeCheck courseId="the-vendasta-platform" site="vendasta_learn"
  sessionSize={3}
  intro="Three quick questions on the model, the two workspaces, where things happen for you versus for a client, and who on your team sees what."
  questions={[
    {
      type: 'mcq',
    ...
    {
      type: 'mcq',
      question: 'A new operations manager needs most of Partner Center, but should not be able to create other Admins. How do you set them up?',
      options: [
        'Give them Admin, then restrict that one area in their permissions',
        'Give them Salesperson, which leaves administration out entirely',
        'Give them Digital Agent, then add the areas they need',
        'Share an existing Admin login with them for the areas they need'
      ],
      correctIndex: 0,
      explanation: 'Permissions go finer than the roles themselves, and an Admin can be restricted from specific areas such as creating other Admins. That is why most of your team starts with some admin access rather than the narrower role.'
    }
  ]}
/>

<LessonFooter to="/learn/getting-started/connect-your-domain-and-email" pathName="Get set up" step={1} totalSteps={9} />
