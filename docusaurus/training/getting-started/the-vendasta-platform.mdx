...
**SMS registration lives with the client.** United States carriers require a business to register before it sends SMS messages (the registration is called A2P, application-to-person). The registration belongs to the business sending the texts, so it happens in that client's Business App, under `Administration` → `Conversation Settings`. [This guide covers the registration](/business-app/conversations/sms/us-businesses-sms-registration).
