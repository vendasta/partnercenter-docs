---
title: "AI Employees and automations, together"
sidebar_position: 6
...
<InlineHighlighter courseId="ai-employees-and-automations-together" site="vendasta_learn">

<CourseProgressBar courseId="ai-employees-and-automations-together" site="vendasta_learn" />
...
There is a sharp edge on the first half: if your automation requires manual input, it is not an automation. A workflow that pauses for a human to fill something in is a task list wearing a costume. Design automations to run start to finish on their own, and give the judgment calls to a worker built for them.

</LessonSection>

## Better together

The two systems are at their best composed, because each hands the other exactly what it lacks. Here is the composition you will build most often, end to end:
...
Asking for the review is rule-shaped: every completed job gets the same request, reliably, with nobody remembering to send it. Responding to the review is judgment-shaped: a glowing five-star note and a lukewarm three-star one deserve different words, and the [AI Reputation Specialist](/learn/grow-your-ai-workforce/ai-reputation-specialist) writes each response to fit. One outcome, a review engine that runs itself, built from two systems doing what each does best.
...
The full vocabulary fits in five lines, and they are the five sections of the `Configure` page you meet in the next path, in the order you meet them.

- **Triggers** are how work reaches it: the events that invoke the employee, one for each channel or platform action you switch on.
- **Role** is who it is: the job description in its `Profile` that sets tone and guidelines for every conversation.
- **Capabilities** are what it does: the instructions that shape its behavior, each one optionally carrying a tool that reaches into another system.
- **Knowledge sources** are what it knows: the business facts it pulls from to keep answers accurate.
- **Automations** are the deterministic workflows on either side of it, starting the employee and started by it.

Every requirement a business brings you belongs to one of those five.

</LessonSection>

<SectionFeedback courseId="ai-employees-and-automations-together" site="vendasta_learn" section="content" />

<KnowledgeCheck courseId="ai-employees-and-automations-together" site="vendasta_learn"
  sessionSize={3}
...
<MarkComplete courseId="ai-employees-and-automations-together" site="vendasta_learn" />
