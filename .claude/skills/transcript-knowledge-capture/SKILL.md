...
description: >-
  ...
  persistent per-topic knowledge base in the enablement-docs repo at
...
`enablement-docs`, the team brain repo. See the Output boundary section below.
...
The **distilled, redacted** output lives in `enablement-docs` at `_source-archive/learn-tab-revamp/transcript-notes/` and gets committed there.
...
skill used to write into moved to `enablement-docs` on 2026-09-01; captured
notes go there now. Redaction on the way in still applies to every entry,
including ones filed before the rule existed.)* If the user points at a file
...
- **Never write anything into this repository from this skill.** Output goes only to `enablement-docs` at `_source-archive/learn-tab-revamp/transcript-notes/`. If that repo is not available, stop and say so — do not fall back to writing here.
