...
13. **Speak to "you." This is the hardest rule in the file.** The learner IS the partner: never "partners can," "agencies," or "the agency" when it means the reader. "Partner Center is where you streamline operations," not "Partner Center streamlines operations for agencies." Their businesses are "your clients."

    **Aggregate framing is the same violation and is banned outright.** Never "most partners," "many partners," "some partners," "plenty of partners," "a lot of partners," "other partners," "partners often/usually/tend to," or any construction that describes the reader as a category of person being observed from outside. It reads as Vendasta talking about its customers with a customer in the room. (Cal, 2026-09-03: "STOP talking to them like they aren't the partner. They ARE the partner. Never say 'most partners' or anything like that EVER.")

    These sentences almost always exist to soften a recommendation or excuse a task's low priority, and they are all deletable. Three ways out, in order of preference: cut the clause and keep only what the reader does ("Most partners arrive with their team already in the platform, so this is housekeeping" → delete, keep the guide link); address them directly ("most partners just confirm it and move on" → "give it a look before you start"); or name the moment instead of the crowd ("most partners set prices when they build packages" → "the natural moment to set your own prices is when you build the packages you actually sell").

    Third person is fine only inside quiz distractors that name other actors, and when naming Vendasta's own programs to their audience is unavoidable — and usually it is avoidable ("Vendasta hosts live training webinars for partners" → "Vendasta hosts live training webinars").
...
6. **Body sections.** In a MIXED-workspace path, any section specific to one workspace opens with an italic location marker: `*You are here: Partner Center. Everything in this section happens in your workspace.*` Sections that span both stay unmarked. When an ENTIRE path happens in one workspace, never repeat the marker per lesson: state it once as a `:::tip` callout near the top of the path's landing page ("Everything in these lessons is done from your main administration workspace: **Partner Center**") and keep the lessons clean (Cal, 2026-08-23).
...
## Navigation and UI references (added 2026-08-23, Cal)

35. **Navigation renders as code, matching the docs side.** Every navigational reference (menu paths, tabs, settings screens, anything the reader clicks through) uses backticks per element with arrows between: `Partner Center` → `Marketing` → `Email Settings`. Never bold for navigation. On-page section names the reader locates (not clicks) stay bold: the **Email Domains** section. This overrides the older "bold for UI elements" line in the repo authoring contract for navigation specifically.
36. **Name the exact location, every time.** Never "under the advanced settings on the same page" or "in the settings area" — give the full path and the named section on every instruction, even when the reader is already on that screen. If the exact label is unknown, find it in the docs side or verify in the platform before writing; a vague location phrase is never acceptable as a placeholder.
37. **Inline the sitting, link the later.** Inline anything the partner must do or decide during this sitting; link out anything that governs what they do later (campaign-time rules, reference detail, legal fine print). A requirement that changes whether this sitting's setup succeeds is never link-only.
38. **Vendasta imagery whenever possible.** Before inventing a glyph or illustration, check the brand assets at `~/mydev/reference/brand-assets/` (AI Workforce faces, Business App shots, AI icons, logos). Anything the product personifies (an AI Employee) uses its actual face, not an abstract icon. Resize copies into the lesson's `img/` (e.g. `sips -Z 288`), never link the reference folder directly, and never commit the multi-MB originals.
39. **Interaction lead-ins name the actual gesture.** The sentence introducing an interactive element describes what the reader physically does with it: flip cards get "Flip each one", tabs get "Click through each one", an accordion gets "Expand each". Never a generic "Pick each one" or "See below" that mismatches the interaction.
40. **Skill-check pages carry no prose.** Header (with outcomes) then the quiz component, nothing between. Meta-commentary about the questions ("They are scenarios rather than definitions...") is redundant with the outcomes and gets cut (Cal, 2026-08-26).
41. **Name the workspaces before any shorthand refers to them.** "Either workspace," "both workspaces," or any paired reference only works after the step has introduced Partner Center and Business App as the pair. Before that point, name them explicitly ("in both Partner Center and Business App") or cut the claim entirely; a first-time reader has no referent for the shorthand. This generalizes: no concept's first contact happens through a shorthand that assumes it is already known. (Cal, 2026-08-28: an Automation flip card said "Automations live in either workspace" before the workspaces section existed on the page; the line was deleted rather than expanded.)
42. **The lead-out after a concept set covers the whole set.** The paragraph that follows a flip-card grid, tab set, or table speaks for every element the set introduced; it never narrates a subset under a collective word that miscounts ("that pairing" after three cards). Either compose all the elements into the takeaway or name the subset explicitly. (Cal, 2026-08-28: three building-block cards were followed by "That pairing is the pattern to remember" covering only two of them; rewritten as a chain of all three.)
43. **A "Try it now" is a real action in the platform, or a reflection that lands the value in one breath.** Never a homework errand outside the product ("count the days since the last post", "check the date on the newest post" — both deleted on sight, Cal 2026-08-28). A pure reflection survives only when it needs no legwork and lands the consequence immediately: "think of the last review a business you know left unanswered for weeks" was deleted with the other two and then restored as "actually good" (Cal, same day) — the difference is instant recall versus an errand. When in doubt, real action or nothing; rule 7's "at least one" yields to this.
44. **Callouts do not stack.** Never more than two admonitions in view in one section; three in a row is callout soup (Cal, 2026-08-28). A closing pointer to setup documentation is a plain paragraph, the last line before the knowledge check, not a callout: reserve boxes for the edition/plan gate (`:::info`, titled) and the Try it now (`:::tip`).
46. **Never describe what is absent.** No "there is no separate test page to find," no "there is nothing to set," no "you will not find one in the Marketplace on its own" once the positive statement has been made. Say where the thing is and what it does; a learner who was not hunting for the missing thing has now been told to wonder about it. A negative survives only when it is the fact itself: a plan gate, a scope boundary ("a client texting only within Canada has nothing to register"), or a rule like "a domain carries only one SPF record." (Cal, 2026-09-03.)

47. **Say it once.** Do not restate the previous sentence in different words, do not repeat on the page what a flip card or knowledge check already carries, and never close a paragraph by explaining what the paragraph was for ("so this is about focus rather than restriction," "for now, the pattern is the takeaway," "Reading first is the habit worth forming"). If the point needed a second sentence to land, the first one is wrong. (Cal, 2026-09-03.)

48. **Write to the partner, not from the transcript.** Reasoning lifted out of a review call reads as reasoning: "treat 72 hours as the outer edge rather than the wait to expect" is a rationale, where "most records validate within minutes, and some take closer to 48 or 72 hours" is what you would actually say. Watch the "X rather than Y" construction in particular; it is almost always the writer explaining a decision instead of stating the fact. (Cal, 2026-09-03, on this pass's own drafting.)

49. **Round portrait images carry no frame.** AI employee face portraits render as bare circles (`width` + `borderRadius: '50%'` only); the repo's standard image style (border, shadow) applies to screenshots and diagrams, not portraits. Confirmed as the intended pattern on PR 1073 review (2026-08-28).
